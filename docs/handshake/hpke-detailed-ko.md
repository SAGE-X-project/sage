...
SAGE의 HPKE 핸드셰이크는 총 4개의 Phase로 구성됩니다:
1. **Invitation** (초대): "핸드셰이크 시작하자" 제안
2. **Request** (요청): HPKE 키 교환 수행
3. **Response** (응답): 키 확인 및 세션 ID 발급
4. **Complete** (완료): 키 검증 및 세션 시작

각 단계를 실제 코드와 함께 살펴봅니다.

---

### Phase 1: Invitation (초대)

**목적**: Agent A가 Agent B에게 핸드셰이크를 시작하겠다고 알림

**코드 위치**: `handshake/client.go:81-94`
