...
| 3 | Two proof schemes exist and neither matches the other. The text signs `"SAGE-PoP:" ‖ DID ‖ ":" ‖ hex(key)` with the key being registered; the contract recovers an Ethereum signature over `"SAGE Agent Registration:" ‖ chainId ‖ contract ‖ owner`, which proves the owner signed, not that the key holder did | `sage-contracts/ethereum/contracts/AgentCardRegistry.sol:418-465` | Security, high confidence |
...
Defects 1 to 4 are reasons to change the wire format before the text is
frozen, not after.
...
registry that cannot verify a type does not accept keys of that type. The
digest convention follows the key's own chapter rather than being special.
