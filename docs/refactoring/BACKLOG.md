...
| D-02 | Phase 1: remove `crypto.Manager`/`internal/cryptoinit` (keep `Set*Constructors` as deprecated no-ops), metrics interfaces + `pkg/telemetry`, `pkg/health` decoupling, `EnhancedProvider` move, no `init()` registration | [중요] | Phase 1, `DECISIONS.md` 2 | In progress. PR: steps 1.2/1.3 first half (`internal/metrics` -> `pkg/telemetry/metrics`, `internal/logger` -> `pkg/telemetry/logger`, `SageError` dropped; 4 of 5 layer violations gone). Open: consumer-side `Metrics` interfaces with `NopMetrics` (1.2), `pkg/health` local logger interface + `slog` default (1.3), 1.1 `crypto.Manager`/`cryptoinit` removal (46 call sites, mostly tests/examples), 1.4 `EnhancedProvider`, 1.5 `init()` registrations |
