...
| D-03 | Phase 2: single sources of truth (chain enum/presets, DID parser, key ID helper, algorithm table, wire codec, replay guard, `AgentMetadata`, version, key-file loader) | [중요] | Phase 2 | In progress. Done/PR: 2.2 DID parser (`did.ParseChain` + `ParseDID`; resolver and CLI use them), 2.4 algorithm table (static in `crypto`, D-02), 2.5 wire codec (`transport.WireMessage`/`WireResponse` + converters shared by HTTP and WebSocket), 2.8 version (`core.Version`, `did.Version`, `sage-verify` const and `lib` literal removed; `pkg/version` only), 2.9 key-file loader (B-02). Open: 2.1 chain presets, 2.3 key id / address helpers, 2.6 replay guard unification, 2.7 `AgentMetadata` unification |
