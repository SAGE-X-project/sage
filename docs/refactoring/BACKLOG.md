...
| A-12 | Dependabot coverage for `tools/codegraph`, `sdk/typescript`, maven, cargo, pip; commit `Cargo.lock`; add TS lockfile; fix nonexistent reviewers team | [권장] | F16, F17 | Partly in PR (pip/npm/cargo/maven for `sdk/`; `Cargo.lock` and TS lockfile committed). Open: `tools/codegraph` gomod entry, reviewers team |
...
| D-04 | Phase 3: `did` interfaces (`Registry`/`Resolver`/`Lister`), `EthereumClient` as deprecated wrapper over `AgentCardClient`, `KeyIDBinder` single definition, `SignatureVerifier` in `crypto` | [중요] | Phase 3, `DECISIONS.md` 2, 5 | In progress. PR: 3.3 `keys.VerifySignature` (Ed25519 / secp256k1 Keccak / P-256, raw or DER) used by `hpke`, the CLI `verify`; `hpke.SignatureVerifier` types delegate and are Deprecated. 3.2 `handshake.KeyIDBinder` = `hpke.KeyIDBinder`. 3.5 `storage.ErrNotFound`/`ErrAlreadyExists`/`ErrExpired`/`ErrNonceUsed` returned wrapped by memory and PostgreSQL stores and asserted by the conformance suite. Open: 3.1 `did` interface set (`Lister`, `EthereumClient` over `AgentCardClient`, global manager), 3.2 `Session` interface trim, 3.4 `chain.Registry` removal, 3.5 unexport per-store types |
...
| D-08 | Mark `sdk/*` experimental; remove interoperability claims; add build CI; TS to 0.x | [중요] | `DECISIONS.md` 4 | PR (experimental banner in the four SDK READMEs and the root README table; TypeScript 1.0.0 -> 0.1.0; `sdk.yml` runs pytest, vitest, `cargo test --locked`, `mvn test` (Java `continue-on-error` until first green run); TS `package-lock.json` and Rust `Cargo.lock` committed; Dependabot pip/npm/cargo/maven for `sdk/`). Follow-up: `tsc --noEmit` fails in `src/react` (peer dependency, unused vars) |
...
| E-03 | `docs/INDEX.md`: 17 dead targets, missing newer docs, wrong `deployment-verify` description | [권장] | §5 priority 1 | PR (`tools/scripts/gen-docs-index.py` generates the index from every Markdown file with the DOCS_GRAPH freshness verdict; `make docs-index` / `docs-index-check` in the Lint job; 0 dead links, all documents listed) |
| E-04 | Retire V2/V4-era documents (`pkg/agent/did/README.md`, `docs/did/*`, `docs/SAGE_A2A_INTEGRATION_GUIDE.md`, `docs/audit/*`, verification guides, `DETAILED_GUIDE_PART*`, test sections) | [권장] | §5 priority 2 | PR (A2A guide, audit package and TEST_EXECUTION_GUIDE archived under `docs/archive/2026-09/` with banners; status banners on `pkg/agent/did/README.md`, `DETAILED_GUIDE_PART{3,5,6B}_KO`, `SECTION_3_DID`, verification guides, `api/*`, `docs/API.md`, `deployments/README.md`, `INSTALL.md`). Open: rewrite `pkg/agent/did/README.md` and `docs/did/*` from cobra `--help` and `AgentCardClient` |
| E-05 | Archive planning-era design docs (`docs/dev/*`, `docs/planning/*`, optimisation plans) under `archive/` with dates | [권장] | §5 priority 3 | Done in PR (`docs/dev/{README,architecture,development-guide,api-spec}`, `docs/planning/*`, `docs/test/{OPTIMIZATION-PLAN,PERFORMANCE-BASELINE,LOAD-TESTING}`, `DOCUMENTATION_AUDIT_2025-10-26`, `contracts-submodule-setup` moved to `docs/archive/2026-09/`; README-only example directories deleted; links rewritten) |
...
| E-07 | Fix 33 contradictions listed in `DOCS_GRAPH.md` §7 (CLAUDE.md handshake description, README code samples, test counts) | [권장] | §7 | PR fixes #3, #4, #5, #6, #9, #10, #13, #14, #15, #16, #17 (E-01), #18, #19 (D-08), #20 (E-03), #22, #23, #24, #25, #29, #30, #31, #32, #33 (scans now blocking). Open: #1 (CLAUDE.md is not tracked), #2/#8 (handshake docs, resolved by D-06), #7 (benchmark figures), #11/#12 (archived), #21 (deployments YAML keys), #26/#27/#28 (contract docs) |
