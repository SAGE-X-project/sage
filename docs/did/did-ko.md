...
- **멀티체인 지원**: Ethereum과 Solana 블록체인 통합
...
├── manager.go            # DID 매니저 구현
├── verification.go       # 메타데이터 검증
├── utils.go              # 유틸리티 함수
├── ethereum/             # Ethereum 블록체인 클라이언트
│   ├── client.go        # Ethereum DID 작업
│   ├── resolver.go      # Ethereum 전용 조회
│   └── contract.go      # 컨트랙트 ABI와 주소
└── solana/              # Solana 블록체인 클라이언트
