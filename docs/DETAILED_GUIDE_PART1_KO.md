...
1. Invitation (초대)
   A → B: "안전하게 대화하고 싶어요" (평문, Ed25519 서명)
   목적: 핸드셰이크 시작 알림

2. Request (요청)
   A가 수행:
   - 임시 X25519 키 쌍 생성 (ephemeralA)
   - B의 Ed25519 공개키로 HPKE 암호화
   - 암호화된 메시지 전송

   내용: "나의 임시 공개키는 ephPubA입니다"
   → B만 복호화 가능!

3. Response (응답)
   B가 수행:
   - A의 메시지 복호화 (자신의 개인키로)
   - 임시 X25519 키 쌍 생성 (ephemeralB)
   - A의 공개키로 HPKE 암호화
   - 암호화된 응답 전송

   내용: "나의 임시 공개키는 ephPubB입니다"

4. 공유 비밀 생성
   A와 B 모두:
   - ECDH(ephPrivA, ephPubB) = 공유 비밀
   - HKDF로 세션 키 유도
...
3. 임시 공개키 교환 (암호화된 채널로)

4. 각자 공유 비밀 계산:
   공유비밀 = ECDH(임시_개인키_A, 임시_공개키_B)
   (A와 B가 같은 결과를 얻음!)

5. 공유 비밀에서 세션 키 유도:
   세션키 = HKDF(공유비밀, "암호화용", 32바이트)
   서명키 = HKDF(공유비밀, "서명용", 32바이트)
...
코드 위치: session/session.go:355-398 (Close 함수의 키 삭제 로직)
```
...
**handshake/client.go** (152줄)
```go
// 클라이언트 (요청 측) 구조체
type Client struct {
    a2a.A2AServiceClient  // gRPC 클라이언트
    key sagecrypto.KeyPair // Ed25519 키
}
...
위치: /handshake/client.go:1-152
```

**handshake/server.go** (519줄)
```go
// 서버 (응답 측) 구조체
type Server struct {
    ...
    pending  map[string]pendingState  // 임시 상태
    peers    map[string]cachedPeer    // 피어 캐시
}
