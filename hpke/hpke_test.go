...
func Test_HPKE(t *testing.T) {
	...
	// Receiver(서버): enc로 HPKE 완료 → exporterSecretB (A와 동일해야 함)
	...
	if err != nil { t.Fatalf("sidA: %v", err) }
	sidB, err := session.ComputeSessionIDFromSeed(expB, "sage/hpke v1")
	if err != nil { t.Fatalf("sidB: %v", err) }
