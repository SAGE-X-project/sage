...
func TestNonceManager(t *testing.T) {
	...
	t.Run("GenerateNonce", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "5.1.1", "Nonce Generation (Cryptographically Secure)")

		n, err := GenerateNonce()
		require.NoError(t, err)
		require.NotEmpty(t, n)

		testutil.LogSuccess(t, "Nonce generation successful")
		testutil.LogDetail(t, "Nonce value: %s", n)
		testutil.LogDetail(t, "Nonce length: %d characters", len(n))

		// Specification Requirement: Nonce should be hex-encoded and at least 32 characters (16 bytes)
		decoded, err := hex.DecodeString(n)
		if err == nil {
			testutil.LogDetail(t, "Nonce is hex-encoded: true")
			testutil.LogDetail(t, "Decoded size: %d bytes", len(decoded))
			assert.GreaterOrEqual(t, len(decoded), 16, "Nonce should be at least 16 bytes")
		} else {
			testutil.LogDetail(t, "Nonce encoding: non-hex format")
		}
		...
		testutil.LogSuccess(t, "Nonce uniqueness verified")
		testutil.LogDetail(t, "Second nonce: %s", nonce2)
		...
		testutil.SaveTestData(t, "nonce/nonce_generation.json", testData)
	})

	t.Run("MarkNonceUsed", func(t *testing.T) {
		// Specification Requirement: Nonce tracking for replay attack prevention
		testutil.LogTestSection(t, "5.1.2", "Nonce Usage Tracking")

		ttlDuration := time.Second
		cleanupDuration := time.Second
		m := NewManager(ttlDuration, cleanupDuration)

		testutil.LogDetail(t, "Nonce TTL: %v", ttlDuration)
		testutil.LogDetail(t, "Cleanup interval: %v", cleanupDuration)

		n := "test-nonce"
		testutil.LogDetail(t, "Test nonce: %s", n)

		// Mark nonce as used
		m.MarkNonceUsed(n)
		testutil.LogSuccess(t, "Nonce marked as used")
		...
		testutil.LogSuccess(t, "Nonce usage verification successful")

		usedCount := m.GetUsedNonceCount()
		require.Equal(t, 1, usedCount)
		testutil.LogDetail(t, "Used nonce count: %d", usedCount)
		...
		testutil.LogSuccess(t, "Duplicate nonce detected correctly")
		...
		testutil.SaveTestData(t, "nonce/nonce_usage_tracking.json", testData)
	})
	...
	t.Run("IsNonceUsedExpiresOnCheck", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "5.1.3", "Nonce Expiration on Check")

		m := NewManager(ttl, time.Hour)
		n := "expiring-nonce"

		testutil.LogDetail(t, "Nonce TTL: %v", ttl)
		testutil.LogDetail(t, "Test nonce: %s", n)

		// Mark nonce as used
		m.MarkNonceUsed(n)
		initialCount := m.GetUsedNonceCount()
		testutil.LogDetail(t, "Initial used nonce count: %d", initialCount)
		require.Equal(t, 1, initialCount)

		// Wait for TTL to expire
		sleepDuration := ttl + 20*time.Millisecond
		testutil.LogDetail(t, "Waiting %v for nonce to expire", sleepDuration)
		...
		testutil.LogSuccess(t, "Expired nonce correctly identified as unused")

		finalCount := m.GetUsedNonceCount()
		require.Equal(t, 0, finalCount, "expired nonce should be removed")
		testutil.LogSuccess(t, "Expired nonce removed from tracking")
		testutil.LogDetail(t, "Final used nonce count: %d", finalCount)
		...
		testutil.SaveTestData(t, "nonce/nonce_expiration_check.json", testData)
	})
	...
	t.Run("CleanupLoopPurgesExpired", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "5.1.4", "Automatic Nonce Cleanup Loop")

		m := NewManager(ttl, cleanup)
		n := "cleanup-nonce"

		testutil.LogDetail(t, "Nonce TTL: %v", ttl)
		testutil.LogDetail(t, "Cleanup interval: %v", cleanup)
		testutil.LogDetail(t, "Test nonce: %s", n)
		...
		testutil.LogDetail(t, "Initial used nonce count: %d", initialCount)
		testutil.LogSuccess(t, "Nonce tracked by manager")
		...
		testutil.LogDetail(t, "Waiting %v for cleanup loop to run", sleepDuration)
		time.Sleep(sleepDuration)

		finalCount := m.GetUsedNonceCount()
		require.Equal(t, 0, finalCount, "cleanupLoop should purge expired nonces")
		testutil.LogSuccess(t, "Cleanup loop purged expired nonces")
		testutil.LogDetail(t, "Final used nonce count: %d", finalCount)
		...
		testutil.SaveTestData(t, "nonce/nonce_cleanup_loop.json", testData)
	})
	...
	t.Run("CheckReplay", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "1.2.2", "Nonce Duplicate Detection (CheckReplay)")
		...
		testutil.LogDetail(t, "Generated nonce: %s", n)

		// First use: should be accepted
		isUsedBefore := m.IsNonceUsed(n)
		require.False(t, isUsedBefore, "nonce should not be used initially")
		testutil.LogSuccess(t, "First use: nonce not marked as used")
		testutil.LogDetail(t, "Is used before marking: %v", isUsedBefore)
		...
		testutil.LogSuccess(t, "Duplicate nonce detected successfully")
		testutil.LogDetail(t, "Is used after marking: %v", isUsedAfter)
		...
		testutil.LogSuccess(t, "Replay attack prevention working")
		...
		testutil.SaveTestData(t, "nonce/nonce_check_replay.json", testData)
	})

	// Test 10.1.10: Nonce 만료
	t.Run("Expiration", func(t *testing.T) {
		// Specification Requirement: TTL-based nonce expiration for memory management
		testutil.LogTestSection(t, "10.1.10", "Nonce Expiration (TTL-based)")
		...
		require.Equal(t, 1, initialCount)
		testutil.LogSuccess(t, "Nonce marked as used")
		testutil.LogDetail(t, "Initial count: %d", initialCount)
		...
		testutil.LogSuccess(t, "Nonce tracked before expiry")
		...
		testutil.LogSuccess(t, "Expired nonce correctly identified as unused")
		testutil.LogDetail(t, "Is used after expiry: %v", isUsedAfterExpiry)

		// Verify nonce is removed from tracking
		finalCount := m.GetUsedNonceCount()
		require.Equal(t, 0, finalCount, "expired nonce should be removed")
		testutil.LogSuccess(t, "Expired nonce removed from tracking")
		testutil.LogDetail(t, "Final count: %d", finalCount)
		...
		testutil.SaveTestData(t, "nonce/nonce_expiration.json", testData)
	})
}
