...
func TestMessageBuilder(t *testing.T) {
	t.Run("Build complete message", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "14.1.1", "RFC9421 메시지 빌더 - 완전한 메시지 생성")

		now := time.Now()
		testutil.LogDetail(t, "테스트 파라미터:")
		testutil.LogDetail(t, "  Agent DID: did:sage:ethereum:agent001")
		testutil.LogDetail(t, "  Message ID: msg-001")
		testutil.LogDetail(t, "  Timestamp: %s", now.Format(time.RFC3339Nano))
		testutil.LogDetail(t, "  Nonce: nonce123")
		testutil.LogDetail(t, "  Body: test body")
		testutil.LogDetail(t, "  Algorithm: EdDSA")
		testutil.LogDetail(t, "  Key ID: key-001")

		testutil.LogDetail(t, "빌더 패턴으로 메시지 생성 중...")
		...
		testutil.LogSuccess(t, "모든 필드 검증 완료")
		testutil.LogDetail(t, "생성된 메시지 구조:")
		...
		testutil.LogDetail(t, "  KeyID: %s", message.KeyID)
		testutil.LogDetail(t, "  Headers count: %d", len(message.Headers))
		testutil.LogDetail(t, "  Metadata count: %d", len(message.Metadata))
		testutil.LogDetail(t, "  SignedFields count: %d", len(message.SignedFields))
		...
		testutil.SaveTestData(t, "rfc9421/message_builder_complete.json", testData)
	})

	t.Run("Build with default signed fields", func(t *testing.T) {
		// 명세 요구사항: SignedFields를 명시하지 않을 경우 기본 필드 자동 설정
		testutil.LogTestSection(t, "14.1.2", "RFC9421 메시지 빌더 - 기본 서명 필드")

		testutil.LogDetail(t, "AgentDID만 지정하고 SignedFields 미지정")
		testutil.LogDetail(t, "  Agent DID: did:sage:ethereum:agent001")

		testutil.LogDetail(t, "기본 설정으로 메시지 빌드 중...")
		message := NewMessageBuilder().
			WithAgentDID("did:sage:ethereum:agent001").
			Build()
		testutil.LogSuccess(t, "메시지 빌드 완료")
		...
		testutil.LogSuccess(t, "기본 서명 필드 검증 완료")
		testutil.LogDetail(t, "기본 SignedFields:")
		for i, field := range message.SignedFields {
			testutil.LogDetail(t, "  [%d] %s", i+1, field)
		}
		...
		testutil.SaveTestData(t, "rfc9421/message_builder_default_fields.json", testData)
	})

	t.Run("Build minimal message", func(t *testing.T) {
		// 명세 요구사항: 파라미터 없이 최소한의 메시지 생성 가능
		testutil.LogTestSection(t, "14.1.3", "RFC9421 메시지 빌더 - 최소 메시지")

		testutil.LogDetail(t, "파라미터 없이 빌더만으로 메시지 생성")

		testutil.LogDetail(t, "최소 설정으로 메시지 빌드 중...")
		...
		testutil.LogSuccess(t, "최소 메시지 필수 필드 검증 완료")
		testutil.LogDetail(t, "생성된 최소 메시지:")
		testutil.LogDetail(t, "  Timestamp 자동 생성: %s", message.Timestamp.Format(time.RFC3339Nano))
		testutil.LogDetail(t, "  Headers 초기화: %v", message.Headers != nil)
		testutil.LogDetail(t, "  Metadata 초기화: %v", message.Metadata != nil)
		testutil.LogDetail(t, "  SignedFields 자동 설정: %d개", len(message.SignedFields))
		...
		testutil.SaveTestData(t, "rfc9421/message_builder_minimal.json", testData)
	})

	t.Run("SetBody", func(t *testing.T) {
		// 명세 요구사항: RFC9421 Content-Digest 생성 검증
		testutil.LogTestSection(t, "14.1.4", "RFC9421 메시지 빌더 - Content-Digest 생성")

		testBody := []byte("test message body for content digest")
		testutil.LogDetail(t, "테스트 Body: %s", string(testBody))

		testutil.LogDetail(t, "WithBody()로 메시지 생성...")
		message := NewMessageBuilder().
			WithAgentDID("did:sage:ethereum:agent001").
		...
		testutil.LogSuccess(t, "Body 설정 및 검증 완료")
		testutil.LogDetail(t, "Body 길이: %d bytes", len(message.Body))
		testutil.LogDetail(t, "Body 내용: %s", string(message.Body))
		...
		testutil.SaveTestData(t, "rfc9421/message_builder_set_body.json", testData)
	})
}
...
func TestParseMessageFromHeaders(t *testing.T) {
	t.Run("Parse complete headers", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "14.2.1", "RFC9421 헤더 파싱 - 완전한 헤더")
		...
		testutil.LogDetail(t, "테스트 헤더:")
		testutil.LogDetail(t, "  X-Agent-DID: %s", headers["X-Agent-DID"])
		testutil.LogDetail(t, "  X-Message-ID: %s", headers["X-Message-ID"])
		testutil.LogDetail(t, "  X-Timestamp: %s", headers["X-Timestamp"])
		testutil.LogDetail(t, "  X-Nonce: %s", headers["X-Nonce"])
		testutil.LogDetail(t, "  X-Signature-Algorithm: %s", headers["X-Signature-Algorithm"])
		testutil.LogDetail(t, "  X-Key-ID: %s", headers["X-Key-ID"])
		testutil.LogDetail(t, "  X-Signed-Fields: %s", headers["X-Signed-Fields"])
		testutil.LogDetail(t, "  Content-Type: %s", headers["Content-Type"])

		body := []byte("test body")
		testutil.LogDetail(t, "Body: %s", string(body))

		testutil.LogDetail(t, "헤더에서 메시지 파싱 중...")
		message, err := ParseMessageFromHeaders(headers, body)
		require.NoError(t, err)
		testutil.LogSuccess(t, "메시지 파싱 완료")
		...
		testutil.SaveTestData(t, "rfc9421/parse_complete_headers.json", testData)
	})

	t.Run("Parse minimal headers", func(t *testing.T) {
		// 명세 요구사항: 최소 필수 헤더만으로 메시지 파싱 가능
		testutil.LogTestSection(t, "14.2.2", "RFC9421 헤더 파싱 - 최소 헤더")

		headers := map[string]string{
			"X-Agent-DID": "did:sage:ethereum:agent001",
		}

		testutil.LogDetail(t, "최소 헤더만 제공:")
		testutil.LogDetail(t, "  X-Agent-DID: %s", headers["X-Agent-DID"])

		body := []byte("test body")
		testutil.LogDetail(t, "Body: %s", string(body))

		testutil.LogDetail(t, "최소 헤더로 메시지 파싱 중...")
		message, err := ParseMessageFromHeaders(headers, body)
		require.NoError(t, err)
		testutil.LogSuccess(t, "메시지 파싱 완료")
		...
		testutil.LogSuccess(t, "최소 헤더 파싱 검증 완료")
		testutil.LogDetail(t, "파싱 결과:")
		testutil.LogDetail(t, "  AgentDID: %s", message.AgentDID)
		...
		testutil.SaveTestData(t, "rfc9421/parse_minimal_headers.json", testData)
	})

	t.Run("Parse with invalid timestamp", func(t *testing.T) {
		// 명세 요구사항: 잘못된 timestamp 형식 시 기본값 사용
		testutil.LogTestSection(t, "14.2.3", "RFC9421 헤더 파싱 - 잘못된 Timestamp 처리")

		headers := map[string]string{
			"X-Timestamp": "invalid-timestamp",
		}

		testutil.LogDetail(t, "잘못된 형식의 Timestamp 헤더:")
		testutil.LogDetail(t, "  X-Timestamp: %s", headers["X-Timestamp"])

		testutil.LogDetail(t, "잘못된 timestamp로 메시지 파싱 시도...")
		message, err := ParseMessageFromHeaders(headers, nil)
		require.NoError(t, err)
		testutil.LogSuccess(t, "메시지 파싱 완료 (에러 없음)")

		// Should use default timestamp when parsing fails
		assert.NotZero(t, message.Timestamp)

		testutil.LogSuccess(t, "잘못된 timestamp 처리 검증 완료")
		testutil.LogDetail(t, "파싱 결과:")
		testutil.LogDetail(t, "  파싱 에러 발생: 없음")
		testutil.LogDetail(t, "  Timestamp 자동 설정: %s", message.Timestamp.Format(time.RFC3339Nano))
		testutil.LogDetail(t, "  Timestamp.IsZero(): %v", message.Timestamp.IsZero())
		...
		testutil.SaveTestData(t, "rfc9421/parse_invalid_timestamp.json", testData)
	})
}

func TestSignatureAlgorithmConstants(t *testing.T) {
	// 명세 요구사항: RFC9421 서명 알고리즘 상수 검증
	testutil.LogTestSection(t, "14.3.1", "RFC9421 서명 알고리즘 상수")

	testutil.LogDetail(t, "지원하는 서명 알고리즘:")
	testutil.LogDetail(t, "  EdDSA (Ed25519)")
	testutil.LogDetail(t, "  ES256K (ECDSA P-256)")
	testutil.LogDetail(t, "  ECDSA (일반)")
	testutil.LogDetail(t, "  ECDSA-secp256k1 (비트코인 커브)")

	testutil.LogDetail(t, "알고리즘 상수 검증 중...")

	assert.Equal(t, SignatureAlgorithm("EdDSA"), AlgorithmEdDSA)
	testutil.LogSuccess(t, "EdDSA 상수 검증 완료")

	assert.Equal(t, SignatureAlgorithm("ES256K"), AlgorithmES256K)
	testutil.LogSuccess(t, "ES256K 상수 검증 완료")

	assert.Equal(t, SignatureAlgorithm("ECDSA"), AlgorithmECDSA)
	testutil.LogSuccess(t, "ECDSA 상수 검증 완료")

	assert.Equal(t, SignatureAlgorithm("ECDSA-secp256k1"), AlgorithmECDSASecp256k1)
	testutil.LogSuccess(t, "ECDSA-secp256k1 상수 검증 완료")
	...
	testutil.SaveTestData(t, "rfc9421/algorithm_constants.json", testData)
}
