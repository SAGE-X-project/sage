...
func TestVerifier(t *testing.T) {
	...
	t.Run("VerifySignature with valid EdDSA signature", func(t *testing.T) {
		...
		testutil.LogDetail(t, "테스트 메시지:")
		testutil.LogDetail(t, "  AgentDID: %s", message.AgentDID)
		testutil.LogDetail(t, "  MessageID: %s", message.MessageID)
		testutil.LogDetail(t, "  Nonce: %s", message.Nonce)
		testutil.LogDetail(t, "  Body: %s", string(message.Body))
		testutil.LogDetail(t, "  Algorithm: %s", message.Algorithm)
		testutil.LogDetail(t, "  SignedFields: %v", message.SignedFields)

		// Sign the message
		testutil.LogDetail(t, "서명 베이스 생성 중...")
		signatureBase := verifier.ConstructSignatureBase(message)
		testutil.LogDetail(t, "서명 베이스 길이: %d bytes", len(signatureBase))

		testutil.LogDetail(t, "Ed25519로 메시지 서명 중...")
		message.Signature = ed25519.Sign(privateKey, []byte(signatureBase))
		testutil.LogSuccess(t, "서명 생성 완료")
		testutil.LogDetail(t, "서명 길이: %d bytes", len(message.Signature))

		// Verify
		testutil.LogDetail(t, "서명 검증 시작...")
		...
		testutil.SaveTestData(t, "rfc9421/verify_valid_eddsa.json", testData)
	})

	t.Run("VerifySignature with invalid signature", func(t *testing.T) {
		// 명세 요구사항: 메시지 변조 감지 - 실제 서명 생성 후 메시지 조작하여 검증 실패 확인
		testutil.LogTestSection(t, "15.1.2", "RFC9421 검증기 - 변조된 메시지 탐지")
		...
		testutil.LogDetail(t, "Step 1: 유효한 메시지 생성")
		testutil.LogDetail(t, "  AgentDID: %s", message.AgentDID)
		testutil.LogDetail(t, "  MessageID: %s", message.MessageID)
		testutil.LogDetail(t, "  Original Body: %q", string(originalBody))

		// Step 2: SAGE Verifier로 실제 서명 생성
		testutil.LogDetail(t, "Step 2: 실제 서명 생성 (SAGE ConstructSignatureBase + Ed25519 Sign)")
		signatureBase := verifier.ConstructSignatureBase(message)
		message.Signature = ed25519.Sign(privateKey, []byte(signatureBase))
		testutil.LogSuccess(t, "유효한 서명 생성 완료")
		testutil.LogDetail(t, "  서명 길이: %d bytes", len(message.Signature))

		// Step 3: 원본 메시지 검증 성공 확인
		testutil.LogDetail(t, "Step 3: 원본 메시지 검증 (정상 통과 예상)")
		err := verifier.VerifySignature(publicKey, message, nil)
		assert.NoError(t, err, "원본 메시지는 검증에 성공해야 함")
		testutil.LogSuccess(t, "원본 메시지 검증 성공")

		// Step 4: 메시지 Body 변조
		tamperedBody := []byte("TAMPERED message content - MODIFIED")
		message.Body = tamperedBody
		testutil.LogDetail(t, "Step 4: 메시지 Body 변조")
		testutil.LogDetail(t, "  Original Body: %q", string(originalBody))
		testutil.LogDetail(t, "  Tampered Body: %q", string(tamperedBody))
		testutil.LogSuccess(t, "메시지 변조 완료")

		// Step 5: 변조된 메시지 검증 실패 확인
		testutil.LogDetail(t, "Step 5: 변조된 메시지 검증 (실패 예상)")
		err = verifier.VerifySignature(publicKey, message, nil)
		...
		testutil.LogSuccess(t, "변조된 메시지 올바르게 거부됨")
		testutil.LogDetail(t, "  에러 메시지: %s", err.Error())
		...
		testutil.SaveTestData(t, "rfc9421/verify_tampered_message.json", testData)
	})

	t.Run("VerifySignature with tampered message - Secp256k1", func(t *testing.T) {
		// 명세 요구사항: 메시지 변조 감지 - Secp256k1 (Ethereum) 서명으로 변조 탐지
		testutil.LogTestSection(t, "15.1.2-2", "RFC9421 검증기 - 변조된 메시지 탐지 (Secp256k1)")

		// Step 1: Secp256k1 (Ethereum) 키 쌍 생성
		testutil.LogDetail(t, "Step 1: Secp256k1 (Ethereum) 키 쌍 생성")
		...
		testutil.LogDetail(t, "Step 2: 유효한 메시지 생성")
		testutil.LogDetail(t, "  AgentDID: %s", message.AgentDID)
		testutil.LogDetail(t, "  MessageID: %s", message.MessageID)
		testutil.LogDetail(t, "  Original Body: %q", string(originalBody))

		// Step 3: SAGE Verifier로 실제 서명 생성 (ECDSA, Ethereum 규약: Keccak-256, r||s||v)
		testutil.LogDetail(t, "Step 3: 실제 서명 생성 (SAGE ConstructSignatureBase + Keccak-256 ECDSA Sign)")
		signatureBase := verifier.ConstructSignatureBase(message)
		signature, err := ethcrypto.Sign(ethcrypto.Keccak256([]byte(signatureBase)), privateKeyEth)
		require.NoError(t, err)
		message.Signature = signature

		testutil.LogSuccess(t, "유효한 서명 생성 완료 (Secp256k1)")
		testutil.LogDetail(t, "  서명 길이: %d bytes", len(message.Signature))

		// Step 4: 원본 메시지 검증 성공 확인
		testutil.LogDetail(t, "Step 4: 원본 메시지 검증 (정상 통과 예상)")
		err = verifier.VerifySignature(publicKeyEth, message, nil)
		assert.NoError(t, err, "원본 메시지는 검증에 성공해야 함")
		testutil.LogSuccess(t, "원본 메시지 검증 성공 (Secp256k1)")

		// Step 5: 메시지 Body 변조
		tamperedBody := []byte("TAMPERED ethereum message - HACKED")
		message.Body = tamperedBody
		testutil.LogDetail(t, "Step 5: 메시지 Body 변조")
		testutil.LogDetail(t, "  Original Body: %q", string(originalBody))
		testutil.LogDetail(t, "  Tampered Body: %q", string(tamperedBody))
		testutil.LogSuccess(t, "메시지 변조 완료")

		// Step 6: 변조된 메시지 검증 실패 확인
		testutil.LogDetail(t, "Step 6: 변조된 메시지 검증 (실패 예상)")
		err = verifier.VerifySignature(publicKeyEth, message, nil)
		assert.Error(t, err, "변조된 메시지는 검증에 실패해야 함")
		assert.Contains(t, err.Error(), "signature verification failed", "에러 메시지에 'signature verification failed' 포함되어야 함")

		testutil.LogSuccess(t, "변조된 메시지 올바르게 거부됨 (Secp256k1)")
		testutil.LogDetail(t, "  에러 메시지: %s", err.Error())
		...
		testutil.SaveTestData(t, "rfc9421/verify_tampered_message_secp256k1.json", testData)
	})

	t.Run("VerifySignature with clock skew", func(t *testing.T) {
		// 명세 요구사항: 시간 동기화 오차 범위 검증
		testutil.LogTestSection(t, "15.1.3", "RFC9421 검증기 - Clock Skew 거부")
		...
		testutil.LogDetail(t, "시간 동기화 오차 테스트:")
		testutil.LogDetail(t, "  현재 시간: %s", time.Now().Format(time.RFC3339))
		testutil.LogDetail(t, "  메시지 Timestamp: %s (10분 후)", futureTime.Format(time.RFC3339))

		opts := &VerificationOptions{
			MaxClockSkew: 5 * time.Minute,
		}
		testutil.LogDetail(t, "검증 옵션:")
		testutil.LogDetail(t, "  MaxClockSkew: %v", opts.MaxClockSkew)

		testutil.LogDetail(t, "Clock skew 초과 메시지 검증 시도...")
		err := verifier.VerifySignature(publicKey, message, opts)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "timestamp outside acceptable range")

		testutil.LogSuccess(t, "Clock skew 초과 메시지 올바르게 거부됨")
		testutil.LogDetail(t, "  에러 메시지: %s", err.Error())
		...
		testutil.SaveTestData(t, "rfc9421/verify_clock_skew.json", testData)
	})

	t.Run("VerifyWithMetadata", func(t *testing.T) {
		// 명세 요구사항: 메타데이터 및 capability 검증
		testutil.LogTestSection(t, "15.1.4", "RFC9421 검증기 - 메타데이터 검증")
		...
		testutil.LogDetail(t, "메타데이터를 포함한 메시지:")
		testutil.LogDetail(t, "  Endpoint: %v", message.Metadata["endpoint"])
		testutil.LogDetail(t, "  Capabilities: %v", message.Metadata["capabilities"])
		...
		testutil.LogDetail(t, "검증 조건:")
		testutil.LogDetail(t, "  Expected endpoint: %v", expectedMetadata["endpoint"])
		testutil.LogDetail(t, "  Required capabilities: %v", requiredCapabilities)

		opts := &VerificationOptions{
			VerifyMetadata: true,
		}

		testutil.LogDetail(t, "메타데이터 검증 중...")
		result, err := verifier.VerifyWithMetadata(publicKey, message, expectedMetadata, requiredCapabilities, opts)
		require.NoError(t, err)
		assert.True(t, result.Valid)
		assert.Empty(t, result.Error)
		testutil.LogSuccess(t, "메타데이터 검증 성공")
		...
		testutil.SaveTestData(t, "rfc9421/verify_with_metadata.json", testData)
	})

	t.Run("VerifyWithMetadata missing capability", func(t *testing.T) {
		// 명세 요구사항: 필수 capability 누락 감지
		testutil.LogTestSection(t, "15.1.5", "RFC9421 검증기 - 누락된 Capability 감지")
		...
		testutil.LogDetail(t, "메시지 capabilities:")
		testutil.LogDetail(t, "  Available: chat=true")
		testutil.LogDetail(t, "  Missing: code")
		...
		testutil.LogDetail(t, "필수 capabilities: %v", requiredCapabilities)

		testutil.LogDetail(t, "누락된 capability로 검증 시도...")
		...
		testutil.LogSuccess(t, "누락된 capability 올바르게 감지됨")
		testutil.LogDetail(t, "  검증 결과: Valid=%v", result.Valid)
		testutil.LogDetail(t, "  에러 메시지: %s", result.Error)
		...
		testutil.SaveTestData(t, "rfc9421/verify_missing_capability.json", testData)
	})

	t.Run("VerifySignature Ed25519", func(t *testing.T) {
		// 명세 요구사항: RFC9421 Ed25519 서명 알고리즘 검증
		testutil.LogTestSection(t, "15.1.6", "RFC9421 검증기 - Ed25519 서명 알고리즘")
		...
		testutil.LogDetail(t, "Ed25519 서명 테스트 메시지:")
		testutil.LogDetail(t, "  Algorithm: %s", message.Algorithm)
		testutil.LogDetail(t, "  AgentDID: %s", message.AgentDID)
		testutil.LogDetail(t, "  MessageID: %s", message.MessageID)

		testutil.LogDetail(t, "Ed25519 서명 생성 중...")
		signatureBase := verifier.ConstructSignatureBase(message)
		message.Signature = ed25519.Sign(privateKey, []byte(signatureBase))
		testutil.LogSuccess(t, "Ed25519 서명 생성 완료")
		testutil.LogDetail(t, "  서명 길이: %d bytes", len(message.Signature))

		testutil.LogDetail(t, "Ed25519 서명 검증 중...")
		err := verifier.VerifySignature(publicKey, message, nil)
		assert.NoError(t, err)
		...
		testutil.SaveTestData(t, "rfc9421/verify_ed25519.json", testData)
	})

	t.Run("VerifySignature ECDSA", func(t *testing.T) {
		// 명세 요구사항: RFC9421 ECDSA 서명 알고리즘 검증
		testutil.LogTestSection(t, "15.1.7", "RFC9421 검증기 - ECDSA 서명 알고리즘")
		...
		testutil.LogDetail(t, "ECDSA 알고리즘 메시지:")
		testutil.LogDetail(t, "  Algorithm: %s", message.Algorithm)
		testutil.LogDetail(t, "  AgentDID: %s", message.AgentDID)
		testutil.LogDetail(t, "  MessageID: %s", message.MessageID)
		...
		testutil.LogSuccess(t, "ECDSA 알고리즘 설정 확인")

		// Verify signature base can be constructed
		testutil.LogDetail(t, "서명 베이스 생성 중...")
		signatureBase := verifier.ConstructSignatureBase(message)
		assert.NotEmpty(t, signatureBase)
		testutil.LogSuccess(t, "서명 베이스 생성 성공")
		testutil.LogDetail(t, "  서명 베이스 길이: %d bytes", len(signatureBase))
		...
		testutil.LogSuccess(t, "ECDSA 메시지 구조 검증 완료")

		testutil.LogDetail(t, "Note: 실제 ECDSA 키 생성 및 검증은 secp256k1/secp256r1 구현 필요")
		...
		testutil.SaveTestData(t, "rfc9421/verify_ecdsa.json", testData)
	})

	// Test 10.1.2: Content-Digest 검증
	t.Run("Digest", func(t *testing.T) {
		// Specification Requirement: Content-Digest validation for message integrity
		testutil.LogTestSection(t, "10.1.2", "RFC9421 Content-Digest 검증")

		bodyContent := []byte("test message for digest verification")
		testutil.LogDetail(t, "테스트 메시지 Body:")
		testutil.LogDetail(t, "  내용: %s", string(bodyContent))
		testutil.LogDetail(t, "  길이: %d bytes", len(bodyContent))

		// Test case 1: Valid digest
		t.Run("valid_digest", func(t *testing.T) {
			testutil.LogDetail(t, "케이스 1: 유효한 Content-Digest")
			...
			testutil.LogDetail(t, "Body의 SHA-256 해시 계산:")
			testutil.LogDetail(t, "  해시: %x", hash)
			testutil.LogDetail(t, "  Base64: %s", expectedDigest)
			testutil.LogDetail(t, "  Content-Digest 헤더: %s", message.Headers["Content-Digest"])
			...
			testutil.LogSuccess(t, "유효한 Content-Digest 검증 시나리오 완료")
			...
			testutil.LogSuccess(t, "Digest 일치 확인 완료")
			...
			testutil.SaveTestData(t, "rfc9421/verify_valid_digest.json", testData)
		})

		// Test case 2: Invalid digest (mismatch)
		t.Run("invalid_digest", func(t *testing.T) {
			testutil.LogDetail(t, "케이스 2: 잘못된 Content-Digest (불일치)")
			...
			testutil.LogDetail(t, "잘못된 Content-Digest 설정:")
			testutil.LogDetail(t, "  Content-Digest 헤더: %s", message.Headers["Content-Digest"])
			...
			testutil.LogDetail(t, "Body의 실제 Digest:")
			testutil.LogDetail(t, "  계산된 Digest: %s", actualDigest)
			testutil.LogDetail(t, "  헤더의 Digest: %s", wrongDigest)

			// Verify digests don't match
			assert.NotEqual(t, actualDigest, wrongDigest)
			testutil.LogSuccess(t, "Digest 불일치 감지 성공")

			// Note: In actual implementation, verifier should reject this
			testutil.LogDetail(t, "Note: 실제 구현에서는 검증기가 이 메시지를 거부해야 함")
			...
			testutil.SaveTestData(t, "rfc9421/verify_invalid_digest.json", testData)
		})

		testutil.LogSuccess(t, "Content-Digest 검증 테스트 완료")
	})

	// Test 1.2.1 & 1.2.2: Nonce 생성 및 Replay Attack 방어
	t.Run("NonceGeneration and ReplayAttackPrevention", func(t *testing.T) {
		testutil.LogTestSection(t, "1.2.1 & 1.2.2", "RFC9421 - Nonce 생성 및 Replay Attack 방어")
		...
		testutil.LogDetail(t, "Step 1: SAGE Nonce 생성 (GenerateNonce)")
		generatedNonce, err := nonce.GenerateNonce()
		require.NoError(t, err)
		require.NotEmpty(t, generatedNonce)
		testutil.LogSuccess(t, "Nonce 생성 완료 (SAGE 핵심 기능 사용)")
		testutil.LogDetail(t, "  Generated Nonce: %s", generatedNonce)
		testutil.LogDetail(t, "  Nonce Length: %d characters", len(generatedNonce))

		// Step 2: Create message with nonce (using SAGE's Message structure)
		testutil.LogDetail(t, "Step 2: Nonce를 포함한 메시지 생성")
		...
		testutil.LogSuccess(t, "메시지 생성 완료")
		testutil.LogDetail(t, "  AgentDID: %s", message.AgentDID)
		testutil.LogDetail(t, "  MessageID: %s", message.MessageID)
		testutil.LogDetail(t, "  Nonce: %s", message.Nonce)
		testutil.LogDetail(t, "  SignedFields: %v", message.SignedFields)
		...
		testutil.LogDetail(t, "Step 3: 메시지 서명 (SAGE ConstructSignatureBase + Ed25519)")
		signatureBase := verifierWithNonce.ConstructSignatureBase(message)
		message.Signature = ed25519.Sign(privateKey, []byte(signatureBase))
		require.NotEmpty(t, message.Signature)
		testutil.LogSuccess(t, "메시지 서명 완료 (Ed25519)")
		testutil.LogDetail(t, "  Signature Length: %d bytes", len(message.Signature))
		testutil.LogDetail(t, "  Signature Base includes nonce: %v", strings.Contains(signatureBase, generatedNonce))

		// Step 4: First verification - should succeed
		testutil.LogDetail(t, "Step 4: 첫 번째 메시지 검증 (성공 예상)")
		err = verifierWithNonce.VerifySignature(publicKey, message, nil)
		require.NoError(t, err)
		testutil.LogSuccess(t, "첫 번째 검증 성공")
		testutil.LogDetail(t, "  Nonce는 자동으로 'used'로 마킹됨 (SAGE NonceManager)")

		// Step 5: Verify nonce is marked as used
		testutil.LogDetail(t, "Step 5: Nonce 사용 여부 확인")
		isNonceUsed := nonceManager.IsNonceUsed(generatedNonce)
		require.True(t, isNonceUsed)
		testutil.LogSuccess(t, "Nonce가 'used'로 올바르게 마킹됨")
		testutil.LogDetail(t, "  IsNonceUsed(%s): %v", generatedNonce, isNonceUsed)

		// Step 6: Attempt replay attack with same nonce - should fail
		testutil.LogDetail(t, "Step 6: Replay Attack 시도 (동일 Nonce 재사용)")
		testutil.LogDetail(t, "  새로운 메시지 Body로 동일 Nonce 재사용 시도")
		message2 := &Message{
			AgentDID:     "did:sage:ethereum:agent-nonce-test",
		...
		testutil.LogDetail(t, "  Second MessageID: %s", message2.MessageID)
		testutil.LogDetail(t, "  Second Body: %s", string(message2.Body))
		testutil.LogDetail(t, "  Reused Nonce: %s", message2.Nonce)

		// Step 7: Verify second message fails due to nonce replay
		testutil.LogDetail(t, "Step 7: 두 번째 메시지 검증 (Replay Attack 탐지 예상)")
		err = verifierWithNonce.VerifySignature(publicKey, message2, nil)
		require.Error(t, err)
		require.Contains(t, err.Error(), "nonce replay attack detected")
		testutil.LogSuccess(t, "Replay Attack 올바르게 탐지 및 거부됨")
		testutil.LogDetail(t, "  Error: %s", err.Error())
		...
		testutil.SaveTestData(t, "rfc9421/nonce_replay_attack_prevention.json", testData)
	})
}

func TestConstructSignatureBase(t *testing.T) {
	// 명세 요구사항: RFC9421 서명 베이스 생성
	testutil.LogTestSection(t, "15.2.1", "RFC9421 서명 베이스 생성")
	...
	testutil.LogDetail(t, "메시지 정보:")
	testutil.LogDetail(t, "  AgentDID: %s", message.AgentDID)
	testutil.LogDetail(t, "  MessageID: %s", message.MessageID)
	testutil.LogDetail(t, "  Timestamp: %s", message.Timestamp.Format(time.RFC3339))
	testutil.LogDetail(t, "  Nonce: %s", message.Nonce)
	testutil.LogDetail(t, "  Body: %s", string(message.Body))
	testutil.LogDetail(t, "  SignedFields: %v", message.SignedFields)
...
Content-Type: application/json`
	...
	testutil.LogSuccess(t, "서명 베이스 생성 및 검증 완료")

	testutil.LogDetail(t, "생성된 서명 베이스:")
	for i, line := range strings.Split(result, "\n") {
		testutil.LogDetail(t, "  [%d] %s", i+1, line)
	}
	...
	testutil.SaveTestData(t, "rfc9421/construct_signature_base.json", testData)
}

func TestDefaultVerificationOptions(t *testing.T) {
	// 명세 요구사항: 기본 검증 옵션 설정
	testutil.LogTestSection(t, "15.3.1", "RFC9421 기본 검증 옵션")

	testutil.LogDetail(t, "기본 검증 옵션 생성 중...")
	opts := DefaultVerificationOptions()
	testutil.LogSuccess(t, "기본 검증 옵션 생성 완료")
	...
	testutil.LogSuccess(t, "모든 기본값 검증 완료")
	testutil.LogDetail(t, "기본 검증 옵션:")
	testutil.LogDetail(t, "  RequireActiveAgent: %v", opts.RequireActiveAgent)
	testutil.LogDetail(t, "  MaxClockSkew: %v", opts.MaxClockSkew)
	testutil.LogDetail(t, "  VerifyMetadata: %v", opts.VerifyMetadata)
	testutil.LogDetail(t, "  RequiredCapabilities: %v", opts.RequiredCapabilities)
	...
	testutil.SaveTestData(t, "rfc9421/default_verification_options.json", testData)
}

func TestHasRequiredCapabilities(t *testing.T) {
	// 명세 요구사항: Capability 검증 로직 테스트
	testutil.LogTestSection(t, "15.4.1", "RFC9421 Capability 검증")
	...
	testutil.LogDetail(t, "테스트용 Capabilities:")
	testutil.LogDetail(t, "  chat: true")
	testutil.LogDetail(t, "  code: true")
	testutil.LogDetail(t, "  voice: false")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			testutil.LogDetail(t, "테스트 케이스: %s", tt.name)
			testutil.LogDetail(t, "  필수 capabilities: %v", tt.required)
			testutil.LogDetail(t, "  예상 결과: %v", tt.expected)
			...
			if result == tt.expected {
				passedCount++
				testutil.LogSuccess(t, "테스트 케이스 통과")
			}
			testutil.LogDetail(t, "  실제 결과: %v", result)
		})
	}

	testutil.LogSuccess(t, "모든 Capability 검증 테스트 완료")
	testutil.LogDetail(t, "통과한 테스트: %d/%d", passedCount, len(tests))
	...
	testutil.SaveTestData(t, "rfc9421/has_required_capabilities.json", testData)
}

func TestCompareValues(t *testing.T) {
	// 명세 요구사항: 값 비교 로직 테스트
	testutil.LogTestSection(t, "15.5.1", "RFC9421 값 비교 로직")
	...
	testutil.LogDetail(t, "값 비교 테스트 케이스 %d개", len(tests))

	passedCount := 0
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			testutil.LogDetail(t, "테스트 케이스: %s", tt.name)
			testutil.LogDetail(t, "  값1: %v", tt.v1)
			testutil.LogDetail(t, "  값2: %v", tt.v2)
			testutil.LogDetail(t, "  예상 결과: %v", tt.expected)
	...
	testutil.LogSuccess(t, "모든 값 비교 테스트 완료")
	testutil.LogDetail(t, "통과한 테스트: %d/%d", passedCount, len(tests))
	...
	testutil.SaveTestData(t, "rfc9421/compare_values.json", testData)
}

func TestSigner(t *testing.T) {
	// Generate test keypair
	testutil.LogDetail(t, "Ed25519 키 쌍 생성 중...")
	publicKey, privateKey, err := ed25519.GenerateKey(rand.Reader)
	require.NoError(t, err)
	testutil.LogSuccess(t, "Ed25519 키 쌍 생성 완료")
	...
	t.Run("Parameters", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "15.6.1", "RFC9421 서명 파라미터 검증")

		now := time.Now()
		keyID := "did:key:z6MkpTHR8VNsBxYAAWHut2Geadd9jSwuBV8xRoAnwWsdvktH"
		nonce := "random-nonce-12345"

		testutil.LogDetail(t, "서명 파라미터 설정:")
		testutil.LogDetail(t, "  KeyID: %s", keyID)
		testutil.LogDetail(t, "  Created: %s", now.Format(time.RFC3339))
		testutil.LogDetail(t, "  Nonce: %s", nonce)
		...
		testutil.SaveTestData(t, "rfc9421/signer_parameters.json", testData)
	})
}
