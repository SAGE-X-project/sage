...
func TestCanonicalizer(t *testing.T) {
	...
	t.Run("basic GET request", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "12.1.1", "RFC9421 정규화 - 기본 GET 요청")

		url := "https://example.com/foo?bar=baz"
		testutil.LogDetail(t, "HTTP 요청 생성:")
		testutil.LogDetail(t, "  메서드: GET")
		testutil.LogDetail(t, "  URL: %s", url)

		req, err := http.NewRequest("GET", url, nil)
		require.NoError(t, err)
		testutil.LogSuccess(t, "HTTP 요청 생성 완료")

		components := []string{`"@method"`, `"@authority"`, `"@path"`, `"@query"`}
		testutil.LogDetail(t, "커버된 컴포넌트: %v", components)
		...
		testutil.LogDetail(t, "  KeyID: %s", params.KeyID)
...
"@signature-params": ("@method" "@authority" "@path" "@query");keyid="test-key";alg="ed25519";created=1719234000`
		...
		testutil.LogSuccess(t, "서명 베이스 검증 완료")
		...
		testutil.SaveTestData(t, "rfc9421/canonicalizer_basic_get.json", testData)
	})
	...
	t.Run("POST request with Content-Digest", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "12.1.2", "RFC9421 정규화 - Content-Digest가 있는 POST")
		...
		testutil.LogDetail(t, "HTTP POST 요청 생성:")
		testutil.LogDetail(t, "  URL: %s", url)
		testutil.LogDetail(t, "  Body: %s", body)
		...
		testutil.LogSuccess(t, "HTTP 요청 및 헤더 설정 완료")
		testutil.LogDetail(t, "  Content-Digest: %s", digest)
		testutil.LogDetail(t, "  Date: %s", date)
		...
		testutil.LogSuccess(t, "Content-Digest 및 Date 헤더 검증 완료")
		...
		testutil.SaveTestData(t, "rfc9421/canonicalizer_post_digest.json", testData)
	})
	...
	t.Run("component not found", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "12.1.3", "RFC9421 정규화 - 컴포넌트 누락 에러")

		url := "https://example.com"
		testutil.LogDetail(t, "HTTP GET 요청 생성: %s", url)
		...
		testutil.LogDetail(t, "존재하지 않는 컴포넌트 요청: %v", components)
		testutil.LogDetail(t, "  참고: GET 요청에 content-digest 헤더 없음")
		...
		canonicalizer := NewCanonicalizer()
		testutil.LogDetail(t, "서명 베이스 생성 시도 (에러 예상)...")
		...
		testutil.SaveTestData(t, "rfc9421/canonicalizer_component_not_found.json", testData)
	})
