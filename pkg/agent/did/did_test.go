...
func TestCreateDID(t *testing.T) {
	...
	testutil.LogTestSection(t, "3.1.1", "DID 생성 (did:sage:ethereum:<uuid> 형식)")

	testutil.LogDetail(t, "DID 생성 테스트:")

	// Generate UUID v4 as identifier
	uuidVal := uuid.New()
	require.NotNil(t, uuidVal)
	testutil.LogDetail(t, "  생성된 UUID: %s", uuidVal.String())

	// Use SAGE's GenerateDID function (NOT manual string concatenation)
	did := GenerateDID(ChainEthereum, uuidVal.String())

	testutil.LogSuccess(t, "DID 생성 완료 (SAGE GenerateDID 사용)")
	testutil.LogDetail(t, "  DID: %s", did)
	testutil.LogDetail(t, "  DID 길이: %d characters", len(did))
	...
	testutil.LogSuccess(t, "DID 형식 검증 완료 (SAGE ValidateDID 사용)")

	// Parse DID components
	parts := strings.Split(string(did), ":")
	require.Len(t, parts, 4, "DID should have 4 parts")
	testutil.LogDetail(t, "  DID 구성 요소:")
	testutil.LogDetail(t, "    Method: %s", parts[1])
	testutil.LogDetail(t, "    Network: %s", parts[2])
	testutil.LogDetail(t, "    ID: %s", parts[3])
	...
	testutil.LogSuccess(t, "UUID v4 형식 검증 완료")
	testutil.LogDetail(t, "  UUID 버전: %d", parsedUUID.Version())
	...
	testutil.LogSuccess(t, "중복 DID 생성 검증 완료 (같은 UUID → 같은 DID)")
	testutil.LogDetail(t, "  원본 DID: %s", did)
	testutil.LogDetail(t, "  중복 DID: %s", didDuplicate)
	...
	testutil.LogSuccess(t, "DID 고유성 검증 완료 (다른 UUID → 다른 DID)")
	testutil.LogDetail(t, "  두 번째 DID: %s", did2)
	...
	testutil.SaveTestData(t, "did/did_creation.json", testData)
}
...
func TestDIDDuplicateDetection(t *testing.T) {
	...
	testutil.LogTestSection(t, "3.1.1.2", "중복 DID 생성 시 오류 반환 (중복 등록 시도)")
	...
	testutil.LogDetail(t, "[Step 2] 첫 번째 Agent 등록 시도...")
	...
	testutil.LogDetail(t, "  Transaction Hash: %s", result1.TransactionHash)
	testutil.LogDetail(t, "  Block Number: %d", result1.BlockNumber)
	...
	testutil.LogSuccess(t, "DID 조회 성공")
	testutil.LogDetail(t, "  Agent 이름: %s", agent.Name)
	...
	if err != nil {
		...
		testutil.LogSuccess(t, "중복 등록 시 오류 발생 (예상된 동작)")
		testutil.LogDetail(t, "  에러 메시지: %v", err)

		// Check if it's the expected error type
		if errors.Is(err, ErrDIDAlreadyExists) || strings.Contains(err.Error(), "already") {
			testutil.LogSuccess(t, "중복 DID 에러 확인 (ErrDIDAlreadyExists 또는 유사)")
		} else {
	...
	testutil.SaveTestData(t, "did/did_duplicate_detection.json", testData)
}
