...
func TestFileKeyStorage(t *testing.T) {
	...
	t.Run("StoreAndLoadKeyPair", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "20.1.1", "File Storage - 키 쌍 저장 및 로드")
		...
		// Store the key pair
		testutil.LogDetail(t, "키 쌍 파일로 저장 중... (key: test-key)")
		err = storage.Store("test-key", keyPair)
		require.NoError(t, err)
		testutil.LogSuccess(t, "키 쌍 파일 저장 완료")

		// Verify file was created
		keyFile := filepath.Join(tempDir, "test-key.key")
		testutil.LogDetail(t, "파일 존재 여부 확인: %s", keyFile)
		assert.FileExists(t, keyFile)
		testutil.LogSuccess(t, "키 파일 생성 확인")

		// Load the key pair
		testutil.LogDetail(t, "파일에서 키 쌍 로드 중...")
		loadedKeyPair, err := storage.Load("test-key")
		require.NoError(t, err)
		assert.NotNil(t, loadedKeyPair)
		assert.Equal(t, keyPair.Type(), loadedKeyPair.Type())
		testutil.LogSuccess(t, "키 쌍 로드 완료 - 타입 일치")
		...
		testutil.SaveTestData(t, "storage/file_store_and_load.json", testData)
	})

	t.Run("StoreSecp256k1KeyPair", func(t *testing.T) {
		// 명세 요구사항: Secp256k1 키 타입 파일 저장 및 로드
		testutil.LogTestSection(t, "20.1.2", "File Storage - Secp256k1 키 저장 및 로드")

		testutil.LogDetail(t, "Secp256k1 키 쌍 생성 중...")
		keyPair, err := keys.GenerateSecp256k1KeyPair()
		require.NoError(t, err)
		testutil.LogDetail(t, "키 쌍 생성 완료: %s", keyPair.ID())

		// Store the key pair
		testutil.LogDetail(t, "Secp256k1 키 쌍 파일로 저장 중... (key: secp256k1-key)")
		err = storage.Store("secp256k1-key", keyPair)
		require.NoError(t, err)
		testutil.LogSuccess(t, "Secp256k1 키 쌍 파일 저장 완료")

		// Load the key pair
		testutil.LogDetail(t, "파일에서 Secp256k1 키 쌍 로드 중...")
		loadedKeyPair, err := storage.Load("secp256k1-key")
		require.NoError(t, err)
		assert.NotNil(t, loadedKeyPair)
		assert.Equal(t, crypto.KeyTypeSecp256k1, loadedKeyPair.Type())
		testutil.LogSuccess(t, "Secp256k1 키 쌍 로드 완료 - 타입 확인")
		testutil.LogDetail(t, "로드된 키 타입: %s", loadedKeyPair.Type())
		...
		testutil.SaveTestData(t, "storage/file_store_secp256k1.json", testData)
	})

	t.Run("LoadNonExistentKey", func(t *testing.T) {
		// 명세 요구사항: 존재하지 않는 파일 로드 시 에러 처리
		testutil.LogTestSection(t, "20.1.3", "File Storage - 존재하지 않는 키 로드")

		testutil.LogDetail(t, "존재하지 않는 키 파일 로드 시도... (key: non-existent)")
		...
		testutil.SaveTestData(t, "storage/file_load_nonexistent.json", testData)
	})

	t.Run("DeleteKey", func(t *testing.T) {
		// 명세 요구사항: 키 파일 삭제 동작 확인
		testutil.LogTestSection(t, "20.1.4", "File Storage - 키 파일 삭제")
		...
		// Store the key
		testutil.LogDetail(t, "키 파일 저장 중... (key: delete-test)")
		err = storage.Store("delete-test", keyPair)
		require.NoError(t, err)
		testutil.LogSuccess(t, "키 파일 저장 완료")

		// Verify file exists
		keyFile := filepath.Join(tempDir, "delete-test.key")
		testutil.LogDetail(t, "파일 존재 확인: %s", keyFile)
		assert.FileExists(t, keyFile)
		testutil.LogSuccess(t, "키 파일 존재 확인")

		// Delete the key
		testutil.LogDetail(t, "키 파일 삭제 중...")
		err = storage.Delete("delete-test")
		require.NoError(t, err)
		testutil.LogSuccess(t, "키 파일 삭제 완료")

		// Verify file is gone
		testutil.LogDetail(t, "파일 삭제 확인 중...")
		assert.NoFileExists(t, keyFile)
		testutil.LogSuccess(t, "키 파일이 존재하지 않음 확인")
		...
		testutil.SaveTestData(t, "storage/file_delete.json", testData)
	})

	t.Run("ListKeys", func(t *testing.T) {
		// 명세 요구사항: 파일 스토리지의 키 목록 조회
		testutil.LogTestSection(t, "20.1.5", "File Storage - 키 목록 조회")

		// Create new storage in clean directory
		testutil.LogDetail(t, "새로운 임시 디렉터리 생성...")
		listDir, err := os.MkdirTemp("", "sage-list-test-*")
		require.NoError(t, err)
		defer func() { _ = os.RemoveAll(listDir) }()
		testutil.LogDetail(t, "임시 디렉터리: %s", listDir)

		listStorage, err := NewFileKeyStorage(listDir)
		require.NoError(t, err)
		testutil.LogSuccess(t, "파일 스토리지 생성 완료")
		...
		require.NoError(t, err)
		testutil.LogSuccess(t, "3개의 키 파일 저장 완료")
		...
		testutil.SaveTestData(t, "storage/file_list.json", testData)
	})

	t.Run("InvalidKeyID", func(t *testing.T) {
		// 명세 요구사항: 잘못된 키 ID에 대한 에러 처리
		testutil.LogTestSection(t, "20.1.6", "File Storage - 잘못된 키 ID 처리")
		...
		testutil.LogDetail(t, "경로 구분자 포함 키 ID로 저장 시도: ../invalid/key")
		err = storage.Store("../invalid/key", keyPair)
		assert.Error(t, err)
		testutil.LogSuccess(t, "예상대로 에러 발생")
		testutil.LogDetail(t, "에러: %v", err)

		testutil.LogDetail(t, "백슬래시 포함 키 ID로 저장 시도: invalid\\key")
		err = storage.Store("invalid\\key", keyPair)
		assert.Error(t, err)
		testutil.LogSuccess(t, "예상대로 에러 발생")
		testutil.LogDetail(t, "에러: %v", err)
		...
		testutil.SaveTestData(t, "storage/file_invalid_key_id.json", testData)
	})

	t.Run("CorruptedKeyFile", func(t *testing.T) {
		// 명세 요구사항: 손상된 키 파일 로드 시 에러 처리
		testutil.LogTestSection(t, "20.1.7", "File Storage - 손상된 키 파일 처리")

		// Create a corrupted key file
		corruptedFile := filepath.Join(tempDir, "corrupted.key")
		testutil.LogDetail(t, "손상된 키 파일 생성: %s", corruptedFile)
		err := os.WriteFile(corruptedFile, []byte("corrupted data"), 0600)
		require.NoError(t, err)
		testutil.LogSuccess(t, "손상된 키 파일 생성 완료")

		// Try to load corrupted key
		testutil.LogDetail(t, "손상된 키 파일 로드 시도...")
		...
		testutil.SaveTestData(t, "storage/file_corrupted.json", testData)
	})

	t.Run("FilePermissions", func(t *testing.T) {
		// 명세 요구사항: 키 파일 권한 검증 (0600)
		testutil.LogTestSection(t, "20.1.8", "File Storage - 파일 권한 검증")
		...
		// Store the key
		testutil.LogDetail(t, "키 파일 저장 중... (key: perm-test)")
		...
		testutil.LogDetail(t, "파일 권한 확인: %s", keyFile)
		...
		testutil.LogDetail(t, "예상 권한: %o", expectedPerm)
		testutil.LogDetail(t, "실제 권한: %o", actualPerm)
		assert.Equal(t, expectedPerm, actualPerm)
		testutil.LogSuccess(t, "파일 권한 확인 완료 - 소유자만 읽기/쓰기 가능 (0600)")
		...
		testutil.SaveTestData(t, "storage/file_permissions.json", testData)
	})
}
