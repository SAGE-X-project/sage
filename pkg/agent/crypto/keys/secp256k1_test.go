...
func TestSecp256k1KeyPair(t *testing.T) {
	t.Run("GenerateKeyPair", func(t *testing.T) {
		...
		testutil.LogTestSection(t, "2.1.1", "Secp256k1 Complete Key Lifecycle (Generation + Secure Storage + Verification)")

		// ====================
		// PART 1: 키 생성 (Key Generation)
		// ====================
		testutil.LogDetail(t, "PART 1: 키 생성 (Key Generation using SAGE core functions)")
		testutil.LogDetail(t, "Step 1-1: Generate Secp256k1 key pair (SAGE GenerateSecp256k1KeyPair)")
		testutil.LogDetail(t, "  Using secp256k1.GeneratePrivateKey() - cryptographically secure random")
		keyPair, err := GenerateSecp256k1KeyPair()
		require.NoError(t, err)
		require.NotNil(t, keyPair)
		testutil.LogSuccess(t, "Secp256k1 key pair generated successfully")

		testutil.LogDetail(t, "Step 1-2: Validate generated key type")
		assert.Equal(t, crypto.KeyTypeSecp256k1, keyPair.Type())
		testutil.LogSuccess(t, "Key type confirmed: Secp256k1")
		...
		testutil.LogSuccess(t, "Private key size validated: 32 bytes")
		...
		testutil.LogSuccess(t, "Public key size validated: 65 bytes (uncompressed)")

		testutil.LogDetail(t, "Step 1-4: Generate Ethereum-compatible address")
		ethAddress := ethcrypto.PubkeyToAddress(*ecdsaPubKey)
		assert.Len(t, ethAddress.Hex(), 42, "Ethereum address must be 42 characters")
		testutil.LogSuccess(t, "Ethereum address generated")
		testutil.LogDetail(t, "  Ethereum address: %s", ethAddress.Hex())

		keyID := keyPair.ID()
		assert.NotEmpty(t, keyID)
		testutil.LogDetail(t, "  Key ID (from public key hash): %s", keyID)

		testutil.LogDetail(t, "Step 1-5: Test cryptographic functionality - Sign message")
		...
		testutil.LogSuccess(t, "Signature generated: 65 bytes (Ethereum format)")

		testutil.LogDetail(t, "Step 1-6: Verify generated signature")
		...
		testutil.LogDetail(t, "Step 3-4: Reconstruct Secp256k1 key pair from decrypted data")
		reconstructedPrivKey, err := ethcrypto.ToECDSA(decryptedKeyBytes)
		require.NoError(t, err)
		reconstructedPubKey := &reconstructedPrivKey.PublicKey
		testutil.LogSuccess(t, "Secp256k1 key pair reconstructed from stored data")
		...
		testutil.LogDetail(t, "Step 3-6: Verify Ethereum address consistency")
		reconstructedEthAddress := ethcrypto.PubkeyToAddress(*reconstructedPubKey)
		assert.Equal(t, ethAddress, reconstructedEthAddress)
		testutil.LogSuccess(t, "Ethereum address consistent after reconstruction")
		testutil.LogDetail(t, "  Original address:      %s", ethAddress.Hex())
		testutil.LogDetail(t, "  Reconstructed address: %s", reconstructedEthAddress.Hex())
		...
		testutil.LogDetail(t, "Step 4-3: Verify address recovery from signature")
		recoveredAddress := ethcrypto.PubkeyToAddress(*recoveredPubKey)
		assert.Equal(t, ethAddress, recoveredAddress)
		testutil.LogSuccess(t, "Address recovery successful - Key fully functional after storage/loading")
		...
		testutil.SaveTestData(t, "keys/secp256k1_key_generation.json", testData)
	})

	t.Run("SignAndVerify", func(t *testing.T) {
		// Specification Requirement: Secp256k1 signature/verification (65-byte signature with recovery)
		testutil.LogTestSection(t, "2.4.2", "Secp256k1 Signature and Verification (Ethereum)")
		...
		testutil.LogDetail(t, "Signature size: %d bytes (expected: 65 bytes)", len(signature))
		testutil.LogDetail(t, "Signature (hex): %x", signature)
		testutil.LogDetail(t, "Recovery byte (v): %d", signature[64])
		...
		if err == nil {
			...
			testutil.LogSuccess(t, "Address recovery successful (Ethereum compatible)")
			testutil.LogDetail(t, "Expected address: %s", expectedAddress.Hex())
			testutil.LogDetail(t, "Recovered address: %s", recoveredAddress.Hex())
		}
		...
		testutil.SaveTestData(t, "keys/secp256k1_sign_verify.json", testData)
	})
...
func TestSecp256k1KeyPairBytes(t *testing.T) {
	...
	testutil.LogTestSection(t, "10.2.6", "Secp256k1 Byte Array Conversion (Compressed/Uncompressed)")
	...
	testutil.LogSuccess(t, "Uncompressed public key extracted")
	testutil.LogDetail(t, "Uncompressed public key size: %d bytes", len(uncompressedPubKey))

	// Verify uncompressed size
	assert.Equal(t, 65, len(uncompressedPubKey), "Uncompressed public key should be 65 bytes")
	testutil.LogSuccess(t, "Uncompressed public key size verified (65 bytes)")

	// Get compressed public key (33 bytes)
	compressedPubKey := ethcrypto.CompressPubkey(pubKey)
	testutil.LogSuccess(t, "Compressed public key extracted")
	testutil.LogDetail(t, "Compressed public key size: %d bytes", len(compressedPubKey))

	// Verify compressed size
	assert.Equal(t, 33, len(compressedPubKey), "Compressed public key should be 33 bytes")
	testutil.LogSuccess(t, "Compressed public key size verified (33 bytes)")

	// Get private key bytes
	privKeyBytes := ethcrypto.FromECDSA(privKey)
	testutil.LogSuccess(t, "Private key bytes extracted")
	...
	testutil.LogSuccess(t, "Decompressed public key matches original")
	...
	testutil.LogSuccess(t, "Private key reconstructed from bytes")
	...
	testutil.LogSuccess(t, "Ethereum address consistent across formats")
	testutil.LogDetail(t, "Ethereum address: %s", addrFromUncompressed.Hex())
	...
	testutil.SaveTestData(t, "keys/secp256k1_byte_conversion.json", testData)
}
...
func TestSecp256k1KeyPairEncrypted(t *testing.T) {
	...
	testutil.LogTestSection(t, "2.2.2", "Secp256k1 Encrypted Key Storage with Password")

	// Generate Secp256k1 key pair
	keyPair, err := GenerateSecp256k1KeyPair()
	require.NoError(t, err)
	testutil.LogSuccess(t, "Secp256k1 key pair generated")
	...
	v, err := vault.NewFileVault(tempDir)
	require.NoError(t, err)
	testutil.LogSuccess(t, "FileVault created")
	testutil.LogDetail(t, "Vault directory: %s", tempDir)
	...
	testutil.LogDetail(t, "Encrypting key with passphrase (AES-256-GCM + PBKDF2)...")
	...
	testutil.LogSuccess(t, "Ethereum address consistent after reconstruction")
	testutil.LogDetail(t, "Original address:      %s", ethAddress.Hex())
	testutil.LogDetail(t, "Reconstructed address: %s", reconstructedAddress.Hex())
	...
	testutil.SaveTestData(t, "keys/secp256k1_encrypted_storage.json", testData)
}
