...
func TestX25519KeyPair(t *testing.T) {
	t.Run("GenerateKeyPair", func(t *testing.T) {
		testutil.LogTestSection(t, "2.1.4", "X25519 Key Pair Generation (ECDH)")

		testutil.LogDetail(t, "Step 1: Generate X25519 key pair for ECDH")
		keyPair, err := GenerateX25519KeyPair()
		require.NoError(t, err)
		assert.NotNil(t, keyPair)
		testutil.LogSuccess(t, "X25519 key pair generated successfully")

		testutil.LogDetail(t, "Step 2: Validate key material")
		assert.NotNil(t, keyPair.PublicKey())
		assert.NotNil(t, keyPair.PrivateKey())
		testutil.LogSuccess(t, "Public and private keys validated")
	...
	t.Run("DeriveSharedSecret", func(t *testing.T) {
		testutil.LogTestSection(t, "2.5.1", "X25519 ECDH Shared Secret Derivation")

		testutil.LogDetail(t, "Step 1: Generate two X25519 key pairs (Alice and Bob)")
		a, err := GenerateX25519KeyPair()
		require.NoError(t, err)
		b, err := GenerateX25519KeyPair()
		require.NoError(t, err)
		testutil.LogSuccess(t, "Two key pairs generated (Alice and Bob)")

		testutil.LogDetail(t, "Step 2: Type conversion to X25519KeyPair")
		aKey, ok := a.(*X25519KeyPair)
		require.True(t, ok)
		bKey, ok := b.(*X25519KeyPair)
		require.True(t, ok)
		testutil.LogSuccess(t, "Type conversion successful")

		testutil.LogDetail(t, "Step 3: Derive shared secret (Alice side)")
		s1, err := aKey.DeriveSharedSecret(bKey.PublicBytesKey())
		require.NoError(t, err)
		testutil.LogSuccess(t, "Alice derived shared secret")
		testutil.LogDetail(t, "  Shared secret size: %d bytes", len(s1))

		testutil.LogDetail(t, "Step 4: Derive shared secret (Bob side)")
		s2, err := bKey.DeriveSharedSecret(aKey.PublicBytesKey())
		require.NoError(t, err)
		testutil.LogSuccess(t, "Bob derived shared secret")

		testutil.LogDetail(t, "Step 5: Verify shared secrets match")
		assert.Equal(t, s1, s2)
		testutil.LogSuccess(t, "Shared secrets match - ECDH successful")
