...
func Test_5_1_1_1_RFC9421SignatureGeneration(t *testing.T) {
	testutil.LogTestSection(t, "5.1.1.1", "RFC 9421 서명 생성")

	testutil.LogDetail(t, "RFC 9421 HTTP 메시지 서명 생성 테스트")
	testutil.LogDetail(t, "테스트 시나리오: Ed25519 키로 HTTP 요청 서명")
	...
	testutil.LogDetail(t, "  Public key (hex): %s...", hex.EncodeToString(publicKey)[:32])
	...
	testutil.LogDetail(t, "HTTP 요청 생성:")
	testutil.LogDetail(t, "  Method: GET")
	testutil.LogDetail(t, "  URL: %s", testURL)
	testutil.LogDetail(t, "  Date: %s", currentTime.Format(time.RFC3339))
	...
	testutil.LogDetail(t, "서명 파라미터:")
	testutil.LogDetail(t, "  Key ID: %s", params.KeyID)
	testutil.LogDetail(t, "  Algorithm: %s", params.Algorithm)
	testutil.LogDetail(t, "  Created: %d", params.Created)
	testutil.LogDetail(t, "  Covered components: %v", params.CoveredComponents)

	// Sign request
	verifier := rfc9421.NewHTTPVerifier()
	err = verifier.SignRequest(req, "sig1", params, privateKey)
	require.NoError(t, err)
	testutil.LogSuccess(t, "HTTP 요청 서명 생성 완료")

	// Verify signature headers are present
	signature := req.Header.Get("Signature")
	require.NotEmpty(t, signature, "Signature header must be present")
	testutil.LogDetail(t, "  Signature header: %s", signature[:64]+"...")

	signatureInput := req.Header.Get("Signature-Input")
	require.NotEmpty(t, signatureInput, "Signature-Input header must be present")
	testutil.LogDetail(t, "  Signature-Input header: %s", signatureInput)
	...
	testutil.LogSuccess(t, "Signature-Input 헤더 포맷 검증 완료")
	...
	testutil.SaveTestData(t, "message/5_1_1_1_rfc9421_signature.json", data)
...
func Test_5_1_1_2_SignatureVerificationSuccess(t *testing.T) {
	testutil.LogTestSection(t, "5.1.1.2", "서명 검증 성공")

	testutil.LogDetail(t, "RFC 9421 HTTP 서명 검증 테스트")
	testutil.LogDetail(t, "테스트 시나리오: 유효한 서명 검증 성공")
	...
	testutil.LogDetail(t, "HTTP 요청:")
	testutil.LogDetail(t, "  Method: POST")
	testutil.LogDetail(t, "  URL: %s", testURL)
	testutil.LogDetail(t, "  Body: {\"test\":\"data\"}")
	...
	signature := req.Header.Get("Signature")
	signatureInput := req.Header.Get("Signature-Input")
	testutil.LogDetail(t, "  Signature: %s...", signature[:48])
	testutil.LogDetail(t, "  Signature-Input: %s", signatureInput)

	// Verify the signature (should succeed)
	err = verifier.VerifyRequest(req, publicKey, nil)
	require.NoError(t, err)
	testutil.LogSuccess(t, "서명 검증 성공 ")
	testutil.LogDetail(t, "  검증 결과: 유효한 서명")

	// Verify multiple times (idempotent)
	err = verifier.VerifyRequest(req, publicKey, nil)
	require.NoError(t, err)
	testutil.LogSuccess(t, "서명 검증 멱등성 확인")
	...
	testutil.SaveTestData(t, "message/5_1_1_2_signature_verification.json", data)
...
func Test_5_1_1_3_TamperedMessageVerificationFailure(t *testing.T) {
	testutil.LogTestSection(t, "5.1.1.3", "변조 메시지 검증 실패")

	testutil.LogDetail(t, "변조된 HTTP 메시지 검증 실패 테스트")
	testutil.LogDetail(t, "테스트 시나리오: 변조된 메시지의 서명 검증 실패")
	...
	err = verifier.SignRequest(req, "sig1", params, privateKey)
	require.NoError(t, err)
	testutil.LogSuccess(t, "원본 메시지 서명 생성 완료")

	originalSignature := req.Header.Get("Signature")
	testutil.LogDetail(t, "  원본 Signature: %s...", originalSignature[:48])
	...
	testutil.LogDetail(t, "테스트 1: Date 헤더 변조")
	tamperedTime := currentTime.Add(1 * time.Hour)
	req.Header.Set("Date", tamperedTime.Format(http.TimeFormat))
	testutil.LogDetail(t, "  변조 전: %s", currentTime.Format(time.RFC3339))
	testutil.LogDetail(t, "  변조 후: %s", tamperedTime.Format(time.RFC3339))

	err = verifier.VerifyRequest(req, publicKey, nil)
	require.Error(t, err, "Tampered message should fail verification")
	testutil.LogSuccess(t, "변조된 Date 헤더 검증 실패 확인 ")
	testutil.LogDetail(t, "  검증 오류: %v", err)

	// Restore original Date
	req.Header.Set("Date", currentTime.Format(http.TimeFormat))

	// Test Case 2: Tamper with Host header
	testutil.LogDetail(t, "테스트 2: Host 헤더 변조")
	originalHost := req.Header.Get("Host")
	req.Header.Set("Host", "malicious.example.com")
	testutil.LogDetail(t, "  변조 전: %s", originalHost)
	testutil.LogDetail(t, "  변조 후: %s", req.Header.Get("Host"))

	err = verifier.VerifyRequest(req, publicKey, nil)
	require.Error(t, err, "Tampered message should fail verification")
	testutil.LogSuccess(t, "변조된 Host 헤더 검증 실패 확인 ")
	testutil.LogDetail(t, "  검증 오류: %v", err)
	...
	testutil.LogDetail(t, "테스트 3: Signature 헤더 변조")
	tamperedSig := originalSignature[:len(originalSignature)-10] + "xxxxxxxxxx"
	req.Header.Set("Signature", tamperedSig)
	testutil.LogDetail(t, "  원본 Signature: %s...", originalSignature[:32])
	testutil.LogDetail(t, "  변조 Signature: %s...", tamperedSig[:32])

	err = verifier.VerifyRequest(req, publicKey, nil)
	require.Error(t, err, "Tampered signature should fail verification")
	testutil.LogSuccess(t, "변조된 Signature 헤더 검증 실패 확인 ")
	testutil.LogDetail(t, "  검증 오류: %v", err)

	testutil.LogSuccess(t, "모든 변조 감지 테스트 통과 ")
	...
	testutil.SaveTestData(t, "message/5_1_1_3_tampered_detection.json", data)
...
func Test_5_1_2_1_TimestampValidation(t *testing.T) {
	testutil.LogTestSection(t, "5.1.2.1", "타임스탬프 검증")

	testutil.LogDetail(t, "RFC 9421 타임스탬프 검증 테스트")
	testutil.LogDetail(t, "테스트 시나리오: 허용 범위 내 타임스탬프 검증")
	...
	testutil.LogDetail(t, "테스트 1: 현재 시간 (허용)")
	currentTime := time.Now()
	...
	require.NoError(t, err)
	testutil.LogSuccess(t, "현재 시간 서명 검증 성공")
	testutil.LogDetail(t, "  타임스탬프: %s", currentTime.Format(time.RFC3339))

	// Test Case 2: 2 minutes ago (within 5-minute skew, should pass)
	testutil.LogDetail(t, "테스트 2: 2분 전 (허용 범위 내)")
	...
	require.NoError(t, err)
	testutil.LogSuccess(t, "2분 전 서명 검증 성공 (허용 범위 내)")
	testutil.LogDetail(t, "  타임스탬프: %s", pastTime.Format(time.RFC3339))
	testutil.LogDetail(t, "  시간 차이: 2분")

	// Test Case 3: 2 minutes in future (within 5-minute skew, should pass)
	testutil.LogDetail(t, "테스트 3: 2분 후 (허용 범위 내)")
	...
	require.NoError(t, err)
	testutil.LogSuccess(t, "2분 후 서명 검증 성공 (허용 범위 내)")
	testutil.LogDetail(t, "  타임스탬프: %s", futureTime.Format(time.RFC3339))
	testutil.LogDetail(t, "  시간 차이: +2분")

	testutil.LogSuccess(t, "타임스탬프 검증 테스트 완료 ")
	...
	testutil.SaveTestData(t, "message/5_1_2_1_timestamp_validation.json", data)
...
func Test_5_1_2_2_ExpiredSignatureRejection(t *testing.T) {
	testutil.LogTestSection(t, "5.1.2.2", "만료된 서명 거부")

	testutil.LogDetail(t, "만료된 서명 거부 테스트")
	testutil.LogDetail(t, "테스트 시나리오: 클록 스큐를 벗어난 서명 거부")
	...
	testutil.LogDetail(t, "테스트 1: 10분 전 서명 (허용 범위 초과)")
	...
	err = verifier.SignRequest(req1, "sig1", params1, privateKey)
	require.NoError(t, err)
	testutil.LogSuccess(t, "만료된 서명 생성 완료")
	testutil.LogDetail(t, "  서명 생성 시각: %s", expiredTime.Format(time.RFC3339))
	testutil.LogDetail(t, "  현재 시각과 차이: 10분")

	// Verify with default options (5-minute max skew)
	err = verifier.VerifyRequest(req1, publicKey, nil)
	require.Error(t, err, "Expired signature (10 minutes old) should be rejected")
	testutil.LogSuccess(t, "만료된 서명 거부 확인 ")
	testutil.LogDetail(t, "  검증 오류: %v", err)

	// Test Case 2: Very old signature (1 hour ago, definitely expired)
	testutil.LogDetail(t, "테스트 2: 1시간 전 서명 (명확히 만료)")
	...
	err = verifier.SignRequest(req2, "sig1", params2, privateKey)
	require.NoError(t, err)
	testutil.LogSuccess(t, "1시간 전 서명 생성 완료")
	testutil.LogDetail(t, "  서명 생성 시각: %s", veryOldTime.Format(time.RFC3339))
	testutil.LogDetail(t, "  현재 시각과 차이: 60분")

	err = verifier.VerifyRequest(req2, publicKey, nil)
	require.Error(t, err, "Very old signature (1 hour) should be rejected")
	testutil.LogSuccess(t, "1시간 전 서명 거부 확인 ")
	testutil.LogDetail(t, "  검증 오류: %v", err)

	testutil.LogSuccess(t, "만료 서명 거부 테스트 완료 ")
	...
	testutil.SaveTestData(t, "message/5_1_2_2_expired_rejection.json", data)
