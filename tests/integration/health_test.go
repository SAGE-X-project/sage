...
func Test_9_1_1_1_HealthEndpointResponse(t *testing.T) {
	testutil.LogTestSection(t, "9.1.1.1", "/health 엔드포인트 정상 응답")

	testutil.LogDetail(t, "시스템 헬스체크 엔드포인트 테스트")
	testutil.LogDetail(t, "테스트 시나리오: 헬스체크 정상 응답 확인")

	// Create health checker (without RPC URL for basic system check)
	checker := health.NewChecker("")
	require.NotNil(t, checker)
	testutil.LogSuccess(t, "헬스체크 생성 완료")

	// Perform health check
	status := checker.CheckAll()
	require.NotNil(t, status)
	testutil.LogSuccess(t, "헬스체크 실행 완료")
	...
	testutil.LogDetail(t, "헬스체크 응답:")
	testutil.LogDetail(t, "  Status: %s", status.Status)
	testutil.LogDetail(t, "  Timestamp: %s", status.Timestamp.Format(time.RFC3339))
	...
	testutil.LogSuccess(t, "Status 값 검증 완료")

	// Verify system health is present
	require.NotNil(t, status.SystemStatus, "System status should be present")
	testutil.LogDetail(t, "  System Status: %s", status.SystemStatus.Status)

	// Log error messages if any
	if len(status.Errors) > 0 {
		testutil.LogDetail(t, "  Errors (%d):", len(status.Errors))
		for i, err := range status.Errors {
			testutil.LogDetail(t, "    [%d] %s", i+1, err)
		}
	} else {
		testutil.LogDetail(t, "  Errors: none")
	}

	testutil.LogSuccess(t, "/health 엔드포인트 응답 검증 완료 ")
	...
	testutil.SaveTestData(t, "health/9_1_1_1_health_endpoint.json", data)
...
func Test_9_1_1_2_BlockchainConnectionStatus(t *testing.T) {
	testutil.LogTestSection(t, "9.1.1.2", "블록체인 연결 상태 확인")

	testutil.LogDetail(t, "블록체인 연결 상태 모니터링 테스트")
	testutil.LogDetail(t, "테스트 시나리오: 블록체인 헬스체크 기능 검증")

	// Test with no RPC URL (should handle gracefully)
	testutil.LogDetail(t, "테스트 1: RPC URL 없이 블록체인 상태 확인")
	blockchainStatus := health.CheckBlockchain("")
	require.NotNil(t, blockchainStatus)
	testutil.LogSuccess(t, "블록체인 상태 확인 함수 실행 완료")

	testutil.LogDetail(t, "  블록체인 연결 상태:")
	testutil.LogDetail(t, "    Status: %s", blockchainStatus.Status)
	testutil.LogDetail(t, "    Connected: %v", blockchainStatus.Connected)
	if blockchainStatus.Error != "" {
		testutil.LogDetail(t, "    Error: %s", blockchainStatus.Error)
	}
	if blockchainStatus.ChainID != "" {
		testutil.LogDetail(t, "    Chain ID: %s", blockchainStatus.ChainID)
	}
	if blockchainStatus.BlockNumber > 0 {
		testutil.LogDetail(t, "    Block Number: %d", blockchainStatus.BlockNumber)
	}

	// Verify status structure
	require.NotEmpty(t, blockchainStatus.Status, "Status should not be empty")
	testutil.LogSuccess(t, "블록체인 상태 구조 검증 완료")

	// Test with local RPC URL (optional, may not be running)
	testutil.LogDetail(t, "테스트 2: 로컬 RPC URL로 블록체인 상태 확인")
	localRPC := "http://localhost:8545"
	blockchainStatusLocal := health.CheckBlockchain(localRPC)
	require.NotNil(t, blockchainStatusLocal)
	testutil.LogDetail(t, "  Local RPC Status:")
	testutil.LogDetail(t, "    RPC URL: %s", localRPC)
	testutil.LogDetail(t, "    Status: %s", blockchainStatusLocal.Status)
	testutil.LogDetail(t, "    Connected: %v", blockchainStatusLocal.Connected)

	if blockchainStatusLocal.Connected {
		testutil.LogSuccess(t, "로컬 블록체인 연결 성공")
		testutil.LogDetail(t, "    Chain ID: %s", blockchainStatusLocal.ChainID)
		testutil.LogDetail(t, "    Block Number: %d", blockchainStatusLocal.BlockNumber)
		if blockchainStatusLocal.Latency != "" {
			testutil.LogDetail(t, "    Latency: %s", blockchainStatusLocal.Latency)
		}
	} else {
		testutil.LogDetail(t, "로컬 블록체인 미연결 (예상됨: 로컬 노드 미실행)")
		if blockchainStatusLocal.Error != "" {
			testutil.LogDetail(t, "    Error: %s", blockchainStatusLocal.Error)
		}
	}

	testutil.LogSuccess(t, "블록체인 연결 상태 확인 완료 ")
	...
	testutil.SaveTestData(t, "health/9_1_1_2_blockchain_status.json", data)
...
func Test_9_1_1_3_SystemResourceMonitoring(t *testing.T) {
	testutil.LogTestSection(t, "9.1.1.3", "메모리/CPU 사용률 확인")

	testutil.LogDetail(t, "시스템 리소스 모니터링 테스트")
	testutil.LogDetail(t, "테스트 시나리오: 메모리, CPU, 디스크 사용률 확인")

	// Check system resources
	systemStatus := health.CheckSystem()
	require.NotNil(t, systemStatus)
	testutil.LogSuccess(t, "시스템 리소스 확인 완료")

	// Verify status
	require.NotEmpty(t, systemStatus.Status, "System status should not be empty")
	testutil.LogDetail(t, "시스템 상태:")
	testutil.LogDetail(t, "  Status: %s", systemStatus.Status)

	// Memory information
	testutil.LogDetail(t, "메모리 사용률:")
	testutil.LogDetail(t, "  Used: %d MB", systemStatus.MemoryUsedMB)
	testutil.LogDetail(t, "  Total: %d MB", systemStatus.MemoryTotalMB)
	testutil.LogDetail(t, "  Percent: %.2f%%", systemStatus.MemoryPercent)
	...
	testutil.LogSuccess(t, "메모리 사용률 검증 완료")

	// CPU information
	testutil.LogDetail(t, "CPU 사용률:")
	testutil.LogDetail(t, "  Percent: %.2f%%", systemStatus.CPUPercent)
	...
	testutil.LogSuccess(t, "CPU 사용률 검증 완료")

	// Disk information
	testutil.LogDetail(t, "디스크 사용률:")
	testutil.LogDetail(t, "  Used: %d GB", systemStatus.DiskUsedGB)
	testutil.LogDetail(t, "  Total: %d GB", systemStatus.DiskTotalGB)
	testutil.LogDetail(t, "  Percent: %.2f%%", systemStatus.DiskPercent)
	...
	testutil.LogSuccess(t, "디스크 사용률 검증 완료")

	// GoRoutines information
	testutil.LogDetail(t, "Go 런타임:")
	testutil.LogDetail(t, "  GoRoutines: %d", systemStatus.GoRoutines)
	require.Greater(t, systemStatus.GoRoutines, 0, "GoRoutines should be > 0")
	testutil.LogSuccess(t, "GoRoutines 정보 검증 완료")

	// Error information
	if systemStatus.Error != "" {
		testutil.LogDetail(t, "  Error: %s", systemStatus.Error)
	} else {
		testutil.LogDetail(t, "  Errors: none")
	}

	testutil.LogSuccess(t, "시스템 리소스 모니터링 완료 ")
	...
	testutil.SaveTestData(t, "health/9_1_1_3_system_resources.json", data)
