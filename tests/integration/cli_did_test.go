...
func Test_6_2_1_1_RegisterDIDSuccess(t *testing.T) {
	testutil.LogTestSection(t, "6.2.1.1", "register 명령으로 DID 등록 성공 테스트")

	testutil.LogDetail(t, "sage-did register 명령이 사용하는 기능 검증")
	testutil.LogDetail(t, "테스트 시나리오: Secp256k1 키로 Ethereum DID 생성")
	...
	testutil.LogSuccess(t, "Ethereum 주소 생성 완료")
	...
	testutil.LogSuccess(t, "Agent DID 생성 완료")
	testutil.LogDetail(t, "  DID: %s", agentDID)
	...
	testutil.LogSuccess(t, "DID 포맷 검증 완료")

	// Parse DID components
	parts := strings.Split(didStr, ":")
	require.GreaterOrEqual(t, len(parts), 4, "DID should have at least 4 parts")
	testutil.LogDetail(t, "  DID 구성:")
	testutil.LogDetail(t, "    - Method: %s", parts[1])
	testutil.LogDetail(t, "    - Chain: %s", parts[2])
	testutil.LogDetail(t, "    - Address: %s", parts[3])
	...
	testutil.SaveTestData(t, "cli/6_2_1_1_register_did.json", data)
...
func Test_6_2_1_2_RegisterWithEthereumChain(t *testing.T) {
	testutil.LogTestSection(t, "6.2.1.2", "--chain ethereum 옵션 동작 확인")

	testutil.LogDetail(t, "sage-did register --chain ethereum 명령 검증")
	...
	testutil.LogSuccess(t, "Ethereum Chain DID 생성 완료")
	testutil.LogDetail(t, "  DID: %s", ethereumDID)

	// Verify chain is Ethereum
	require.True(t, strings.Contains(string(ethereumDID), "ethereum"), "DID should contain 'ethereum' chain")
	testutil.LogSuccess(t, "Chain 타입 검증 완료 (ethereum)")
	...
	testutil.LogSuccess(t, "Nonce를 사용한 DID 생성 완료")
	testutil.LogDetail(t, "  DID with nonce: %s", didWithNonce)

	// Verify nonce in DID
	require.True(t, strings.HasSuffix(string(didWithNonce), ":1"), "DID should end with :1")
	testutil.LogSuccess(t, "Nonce 포함 DID 검증 완료")
	...
	testutil.SaveTestData(t, "cli/6_2_1_2_ethereum_chain.json", data)
...
func Test_6_2_2_1_ResolveDIDSuccess(t *testing.T) {
	testutil.LogTestSection(t, "6.2.2.1", "resolve 명령으로 DID 조회 성공 테스트")

	testutil.LogDetail(t, "sage-did resolve 명령이 사용하는 기능 검증")
	testutil.LogDetail(t, "테스트 시나리오: DID 포맷 파싱 및 검증")
	...
	testutil.LogSuccess(t, "테스트용 DID 생성 완료")
	testutil.LogDetail(t, "  DID: %s", testDID)
	...
	testutil.LogSuccess(t, "DID 파싱 성공")
	testutil.LogDetail(t, "  파싱된 구성:")
	testutil.LogDetail(t, "    - Method: %s", method)
	testutil.LogDetail(t, "    - Chain: %s", chain)
	testutil.LogDetail(t, "    - Identifier: %s", identifier)
	...
	testutil.LogSuccess(t, "DID 구성 요소 검증 완료")
	...
	testutil.SaveTestData(t, "cli/6_2_2_1_resolve_did.json", data)
...
func Test_6_2_2_2_ListDIDs(t *testing.T) {
	testutil.LogTestSection(t, "6.2.2.2", "list 명령으로 전체 DID 목록 조회 테스트")

	testutil.LogDetail(t, "sage-did list 명령이 사용하는 기능 검증")
	testutil.LogDetail(t, "테스트 시나리오: 여러 DID 생성 및 필터링")
	...
	address, err := did.DeriveEthereumAddress(keyPair)
	require.NoError(t, err)
	testutil.LogSuccess(t, "테스트용 Ethereum 주소 생성 완료")
	...
	testutil.LogSuccess(t, "다중 DID 생성 완료")
	testutil.LogDetail(t, "  생성된 DID 수: %d", len(dids))

	// Display DIDs (what list command does)
	for i, d := range dids {
		testutil.LogDetail(t, "  [%d] %s", i+1, d)
	}
	...
	testutil.SaveTestData(t, "cli/6_2_2_2_list_dids.json", data)
...
func Test_6_2_3_1_UpdateDIDMetadata(t *testing.T) {
	testutil.LogTestSection(t, "6.2.3.1", "update 명령으로 메타데이터 수정 테스트")

	testutil.LogDetail(t, "sage-did update 명령이 사용하는 기능 검증")
	testutil.LogDetail(t, "테스트 시나리오: DID 메타데이터 구조 검증")
	...
	testutil.LogSuccess(t, "메타데이터 구조 생성 완료")
	testutil.LogDetail(t, "  초기 메타데이터:")
	testutil.LogDetail(t, "    - Name: %s", initialMetadata["name"])
	testutil.LogDetail(t, "    - Endpoint: %s", initialMetadata["endpoint"])

	testutil.LogDetail(t, "  업데이트 메타데이터:")
	testutil.LogDetail(t, "    - Name: %s", updatedMetadata["name"])
	testutil.LogDetail(t, "    - Endpoint: %s", updatedMetadata["endpoint"])
	testutil.LogDetail(t, "    - Version: %s", updatedMetadata["version"])

	// Verify metadata can be updated
	require.NotEqual(t, initialMetadata["name"], updatedMetadata["name"], "Metadata should be updatable")
	testutil.LogSuccess(t, "메타데이터 수정 가능성 검증 완료")
	...
	testutil.SaveTestData(t, "cli/6_2_3_1_update_metadata.json", data)
...
func Test_6_2_3_2_RevokeDID(t *testing.T) {
	testutil.LogTestSection(t, "6.2.3.2", "deactivate 명령으로 DID 비활성화 테스트")

	testutil.LogDetail(t, "sage-did deactivate 명령이 사용하는 기능 검증")
	testutil.LogDetail(t, "테스트 시나리오: DID 상태 변경 검증")
	...
	testutil.LogSuccess(t, "DID 상태 구조 생성 완료")
	testutil.LogDetail(t, "  초기 상태: active = %v", initialState["is_active"])
	testutil.LogDetail(t, "  변경 후 상태: active = %v", deactivatedState["is_active"])
	...
	testutil.LogSuccess(t, "DID 상태 변경 검증 완료")
	...
	testutil.SaveTestData(t, "cli/6_2_3_2_deactivate_did.json", data)
...
func Test_6_2_3_3_VerifyDID(t *testing.T) {
	testutil.LogTestSection(t, "6.2.3.3", "verify 명령으로 DID 검증 테스트")

	testutil.LogDetail(t, "sage-did verify 명령이 사용하는 기능 검증")
	testutil.LogDetail(t, "테스트 시나리오: DID 형식 및 일관성 검증")
	...
	testutil.SaveTestData(t, "cli/6_2_3_3_verify_did.json", data)
