...
### Changed
- RFC 9421 interoperability (sage-spec 03): `@request-target` is now the request target without the method, as RFC 9421 section 2.2.5 defines it (previously `METHOD path?query`, the older draft form, which no other implementation produces); the verifier builds the `"@signature-params"` line from the received `Signature-Input` member byte for byte instead of re-serialising the parsed parameters, so signatures that carry `tag` or parameters in another order now verify; `SignatureInputParams` gains `Tag` and `Raw`, and `rfc9421.FormatSignatureParams` is the single serialisation used by signers. A new `request-target-ed25519` spec vector covers `@request-target`, `@path` and `@query`. Signatures produced by earlier releases over `@request-target` no longer verify; the default component set never included it.
- `did.AgentMetadata` is the single agent metadata type: it carries the full key list (`Keys []AgentKey`) next to the single-key view (`PublicKey`, `PublicKEMKey`), and every resolver (Ethereum legacy client, `AgentCardClient`, Solana) fills both. `AgentMetadataV4` is a deprecated alias, `FromAgentMetadata` / `ToAgentMetadata` are deprecated names for the new `Normalized()` (returns a copy with the missing view filled in; never mutates cached results), and `KEMKey()` returns the raw X25519 bytes. A2A card verification now compares against the resolver's key list instead of a lossy reconstruction, so Ed25519 keys and revoked (unverified) keys are handled the same way as in DID resolution; `AgentCardClient.GetAgent` / `GetAgentByDID` apply the same key policy as the legacy client. Behaviour change: an agent without an ECDSA key now exposes its verified Ed25519 key as `PublicKey` in the single-key view (previously nil).
- `did` interface set trimmed (D-04 3.1). New `did.Lister` (`ListAgentsByOwner`, `Search`) is embedded in `Resolver`, whose method set is unchanged; new `did.KeyRegistry` (`AddKey`, `RevokeKey`, `ApproveEd25519Key`) is what `Manager` asserts for key management, and `RegistryV4` is a deprecated alias of `Registry` + `KeyRegistry`. `did.ChainClient` (`Registry` + `Resolver`) with `RegisterClientCreator(chain, creator)` / `ClientCreatorFor(chain)` replaces the three separate creator hooks (`RegisterEthereumClientCreator`, `RegisterSolanaClientCreator`, `RegisterEthereumV4ClientCreator`), so `Manager.Configure` now wires Solana as well as Ethereum. `Manager` implements `Resolver` (`Resolve`, `ResolveKEMKey`, `VerifyMetadata`, `Search`) and can be passed to the HPKE client and server directly. `SolanaClient` gained `ResolveKEMKey` and its `ResolvePublicKey` returns `interface{}`, so it satisfies `Resolver`. Removed: the `did.Client` interface, `ClientFactory` / `NewClientFactory` / `CreateClient`, the package-level manager (`GetDefaultManager`, `did.Configure`, `did.RegisterAgent`, `did.ResolveAgent`, `did.ValidateAgent`, `did.CheckCapabilities`; construct a `did.NewManager()`), `EthereumV4ClientCreator` and its accessors. `did.GetRecommendedKeyType`, `ValidateKeyTypeForChain` and `GetRFC9421Algorithm` stay as deprecated wrappers over `pkg/agent/crypto/chain`.
- Smaller exported surfaces (D-04 3.2, 3.4, 3.5). `session.Manager` returns `*SecureSession` from `CreateSession`, `GetSession`, `GetByKeyID` and the `Ensure*` methods instead of the `Session` interface; `session.Session` is kept as a deprecated five-method interface (`GetID`, `Encrypt`, `Decrypt`, `SignCovered`, `VerifyCovered`) that `*SecureSession` satisfies, and the unused `Metadata` / `MetadataBuilder` / `GenerateSalt` / `GeneralPrefix` are removed. `chain.ChainRegistry`, `chain.NewRegistry`, `chain.PublicKeyResolver`, `chain.ChainKeyTypeMapper` and `chain.NewChainKeyTypeMapper` are removed (none had callers outside the package); the package-level `RegisterProvider` / `GetProvider` / `ListProviders` / `GenerateAddresses` and the key-type functions are unchanged and now read static tables directly. The per-store types of `pkg/storage/memory` and `pkg/storage/postgres` are unexported (`Store`, `Config` and `NewStore` stay; the accessors already returned the `storage.*` interfaces). `auth0.NewVerifier` returns the now exported `*auth0.Verifier`; the verifier returns the `oidc` sentinels `ErrInvalidIssuer`, `ErrInvalidSigningAlg`, `ErrNoPublicKey` and the new `ErrInvalidClaims`, `ErrNoJWKSKeys` (wrapped, messages unchanged), and its RSA test-key helper moved into a test file.
- `ethereum.AgentCardClient` is the Ethereum `did.ChainClient` (D-04 3.1, decision 5): it now implements `Registry`, `Resolver` and `Lister` keyed by DID (`Resolve`, `ResolvePublicKey`, `ResolveKEMKey`, `VerifyMetadata`, `ListAgentsByOwner`, `GetRegistrationStatus`, `Update`, `Deactivate`), and `did.Manager.Configure` installs it for `did.ChainEthereum`. `EthereumClient` is a deprecated wrapper embedding `*AgentCardClient`: its earlier implementation packed SageRegistryV2 function signatures against the AgentCardRegistry ABI, so `Register`, `Update` and `Deactivate` could never succeed; reads are unchanged in behaviour and now use the generated binding. `Register` on either type returns `ErrCommitRevealRequired` (the contract only registers through `CommitRegistration` / `RegisterAgent` / `ActivateAgent`), `Update` accepts `endpoint` and `capabilities` only (the contract cannot change name or description) and keeps the current value of whichever is omitted, `Search` returns `ErrSearchUnsupported`, and a missing agent is reported as `did.ErrDIDNotFound`. Removed: `SageRegistryABI`, `GetSageRegistryABI` and the embedded `SageRegistryV2.abi.json`.
- Smart contracts moved to the `sage-contracts` repository (github.com/SAGE-X-project/sage-contracts) with their history; `contracts/` is removed here. The Go bindings are generated from the ABI files that repository publishes, at the commit pinned in `.contracts-version` (`make contracts-checkout`, `make bindings`, `make bindings-check`; `CONTRACTS_DIR` overrides the checkout). Contract tests, solhint and Slither run in sage-contracts; the CI here keeps a `Contract Bindings` drift check and starts the Hardhat node for integration tests from the pinned checkout. Dependabot's npm entry for the contracts moved with them.
...
- `docs/INDEX.md` is generated by `tools/scripts/gen-docs-index.py` (`make docs-index`) from every Markdown document in the repository, grouped by area and annotated with the freshness verdict from `docs/refactoring/DOCS_GRAPH.md`; the Lint job fails when it is out of date. The previous hand-maintained index had 18 dead links and omitted 71 documents.
- Examples no longer overstate what they do. `examples/a2a-integration/04-secure-message` encrypts with real HPKE (RFC 9180, `keys.HPKESealAndExportToX25519Peer`) instead of copying the plaintext; the MCP `simple-standalone` and `vulnerable-vs-secure/secure-chat` servers verify RFC 9421 signatures with `rfc9421.HTTPVerifier` strict options against a trusted-key table (previously they only checked that headers were present), and their READMEs say what is checked and what is not (no on-chain resolution, no capability check). `contracts/ethereum/docs/GOVERNANCE-SETUP.md` is marked as a non-executable draft.
- The language clients under `sdk/` (Python, TypeScript, Rust, Java) are marked experimental: their READMEs and the root README state that they are not interoperable with the Go core (no RFC 9421, no Go HPKE handshake, no server for the endpoints they call). The TypeScript package version drops from 1.0.0 to 0.1.0. A new `SDK Build` workflow keeps them compiling and their unit tests green; lockfiles are committed and Dependabot covers the four ecosystems.
- `keys.VerifySignature` is the single signature verification routine (pure Ed25519; secp256k1 with Keccak-256 accepting `r||s`, `r||s||v` or DER; P-256 with SHA-256 accepting raw or DER). The HPKE `ECDSAVerifier`/`Ed25519Verifier`/`CompositeVerifier` delegate to it and are `Deprecated`; `sage-crypto verify` uses it, so secp256k1 signatures are now checked with the Ethereum convention there as well (previously SHA-256). `handshake.KeyIDBinder` is an alias of `hpke.KeyIDBinder`. `pkg/storage` gained sentinel errors (`ErrNotFound`, `ErrAlreadyExists`, `ErrExpired`, `ErrNonceUsed`) that the memory and PostgreSQL stores return wrapped; use `errors.Is`.
- `chain.Presets` / `chain.PresetFor` is the single table of known networks (local, sepolia, ethereum-mainnet, kairos, cypress, solana-devnet, solana-mainnet: chain id, public RPC, deployed registry addresses, with aliases such as `mainnet` -> `cypress`). `deployments/config.NetworkPresets` (now including `sepolia`) and `GetContractAddress` read from it, and `sage-did` defaults to the Sepolia preset (AgentCardRegistry `0xC7eC…a808`, public RPC) instead of a placeholder Alchemy URL and the zero address.
- `keys.KeyID` is the single definition of key identifiers (first 8 bytes of SHA-256 of the public key, hex) used by every key type, and `keys.EthereumAddress` the single Ethereum address derivation, used by `did.DeriveEthereumAddress` and the Ethereum chain provider (both previously re-implemented Keccak-256 hashing).
- One replay guard: `session.ReplayGuard` (`CheckAndMark(scope, nonce)`) with `session.MemoryReplayGuard` (TTL, per-scope `Forget`, background sweep). The RFC 9421 HTTP verifier, the envelope `Verifier` (`NewVerifierWithReplayGuard`; nonces scoped by agent DID), the HPKE server (`ServerOpts.ReplayGuard`; scoped by context id, replacing the O(n) `nonceStore`) and the session manager all use it. `rfc9421.ReplayGuard` and `NonceReplayGuard` are aliases; `session.NonceCache` and `rfc9421.NewVerifierWithNonceManager` remain as `Deprecated` wrappers.
- Single sources of truth: `transport.WireMessage` / `WireResponse` and their converters are the one wire codec used by the HTTP and WebSocket transports (the two private copies are gone); `did.ParseChain` defines the chain aliases used by `did.ParseDID`, the resolver and the `sage-did --chain` flag, and `ParseDID` rejects an empty identifier; the version string comes only from `pkg/version` (`core.Version`, `did.Version`, the `sage-verify` constant and the `lib` literal are removed).
- Library packages no longer register themselves in `init()`. Chain providers (`chain/ethereum`, `chain/solana`), DID clients (`did/ethereum`, `did/solana`) and transports (`transport/http`, `transport/websocket`) expose an idempotent `Register()`; the binaries call `internal/app.RegisterDefaults()` at start. Programs that relied on a blank import (`_ "…/did/ethereum"`) to make `did.Manager.Configure` work must call `ethereum.Register()` instead. The built-in algorithm table is now a static value in `pkg/agent/crypto` (no blank import of `keys` needed); `did.GetDefaultManager`, the embedded ABIs and the Prometheus registry are initialised as package variables. CI fails on any new `func init()` under `pkg/` (`make check-no-init`).
- The retrying EVM provider moved to `pkg/blockchain/ethereum` as `Provider`, configured with `Endpoint` (RPC URL, chain id, gas limit and price cap, retry and timeout settings) instead of `deployments/config.BlockchainConfig`; `BlockchainConfig.Endpoint()` converts. `chain/ethereum.EnhancedProvider`, `EthClient`, `NewEnhancedProvider` and `NewEnhancedProviderWithClient` remain as `Deprecated` aliases; `GetConfig()` is replaced by `Provider.Endpoint()`.
- `session` reports events through the consumer-side `session.Metrics` interface (`NopMetrics` by default; `Manager.SetMetrics`, `SecureSession.SetMetrics`) and no longer imports the Prometheus vectors; `metrics.PrometheusSessionMetrics` is the adapter to install from the composition root (`examples/metrics-demo` does). `health.NewServer` takes a two-method `health.Logger` (`*slog.Logger` satisfies it; nil selects `slog.Default()`), and `/metrics` serves whatever `SetMetricsHandler` installs (404 until then); the JSON collector snapshot moved to `metrics.SnapshotHandler()`. `health` no longer depends on `pkg/telemetry`.
...
### Security
- Ethereum DID resolution now applies a key policy: only keys whose on-chain `verified` flag is set are used (the registry expresses revocation by clearing that flag, so revoked keys are no longer trusted), the signing key falls back to a verified Ed25519 key for agents without an ECDSA key, and a verified X25519 key from the key list is used as the KEM key when the agent record carries none. `ResolvePublicKey` returns `did.ErrNoSigningKey` instead of a nil key.
### Security
...
- Go contract bindings (`pkg/blockchain/ethereum/contracts/agentcardregistry`) are generated by `tools/scripts/gen-bindings.sh` with the abigen matching go.mod's go-ethereum version (`make bindings`), and the Smart Contract Tests job fails when the committed bindings drift from the compiled artifacts (`make bindings-check`). The bindings were regenerated; `AgentCardStorage` gains the `KEMKeyUpdated` event that was missing.
- Layer boundaries are enforced. `depguard` rules in `.golangci.yml` forbid `pkg` from importing `internal`/`cmd`, `internal` from importing `cmd`, anything from importing `examples`/`tests`/`tools`, and lower clusters (`crypto`, then `did`/`session`/`transport`, then `hpke`/`core`) from importing higher ones. The five existing `pkg -> internal` imports are listed as exceptions with the item that removes them. The code-graph tool gained a `-layer-baseline` gate (`make codegraph-check`, run in the Lint job) that fails on any new violation, and `pkg/storage/storagetest.RunConformance` defines the contract every storage backend must pass.
- Releases are now built by GoReleaser (`.goreleaser.yaml`) reproducibly (`CGO_ENABLED=0`, `-trimpath`, commit timestamp as build date) and published with a Sigstore keyless signature of `SHA256SUMS`, SLSA build provenance (GitHub artifact attestations) for every archive and for the container image, a CycloneDX SBOM per archive, and a cosign-signed image. `SECURITY.md` documents how to verify them. Local `make build-platform` uses the same reproducibility flags.
- Transport hardening. The HTTP message server limits request bodies (`DefaultMaxBodyBytes`, 1 MiB, `SetMaxBodyBytes`) and answers 413 for larger ones; the WebSocket server limits inbound messages (`DefaultMaxMessageBytes`, `SetMaxMessageBytes`). WebSocket origin checking is now on by default: browser requests are accepted only from the server's own origin or the allow list, requests without an `Origin` header (non-browser agents) are accepted unless `SetRequireOrigin(true)`, and the allow list is guarded by a mutex (it raced with the upgrader). The `X-SAGE-DID` / `X-SAGE-Message-ID` / `X-SAGE-Context-ID` / `X-SAGE-Task-ID` headers no longer override the JSON body; a header that contradicts the body is rejected. RFC 9421 content-digest validation buffers at most `MaxBodyBytes` (16 MiB). HPKE init rejections are reported to the peer as the single `ErrInitRejected` ("authentication failed"); the reason (skew, replay, addressing, info mismatch) goes to `ServerOpts.Logger`.
- Canonical JSON. New package `pkg/agent/crypto/jcs` implements RFC 8785 (JSON Canonicalization Scheme). A2A card proofs are now computed over the canonical card without `proof` (Ed25519 signs the bytes directly; secp256k1 signs Keccak-256 of them, the Ethereum convention), and `ParseA2AAgentCardWithProof` keeps the received JSON so verification covers exactly what arrived. The HPKE handshake response is signed over the canonical form of the members it carries, and the client verifies over the canonical form of the bytes it received (minus `sigB64`) after checking `did` and the echoed `enc`/`ephC`; previously the client re-encoded a Go struct, which no other language could reproduce. Proofs and responses produced by earlier releases no longer verify.
- `GetKeyTypeFromPublicKey` distinguishes P-256 from secp256k1 by curve; P-256 keys were previously reported as secp256k1, so `ecdsa-p256-sha256` signatures were rejected and `es256k` accepted for the wrong curve. All ECDSA signers (P-256 key pairs, RFC 9421 P-256 signing) emit low-S signatures.
- Audience binding. The HPKE server rejects an init whose `respDid` is not its own DID (previously any correctly signed init created a session and bound a key id). RFC 9421 verification gained `ExpectedDID` / `ExpectedKeyID` (the signature's `keyid` must identify a key of the DID whose public key was resolved) and, for requests, `ExpectedAuthorities` (`@authority` must be covered and match one of the verifier's own authorities, so a request signed for agent A cannot be forwarded to agent B). `KeyIDDID` extracts the DID from a `keyid`. The MCP example tool binds `keyid` to the resolved DID.
- RFC 9421 response signing: `HTTPVerifier.SignResponse` / `VerifyResponse` (and `Verifier.SignHTTPResponse` / `VerifyHTTPResponse`) sign and verify HTTP responses, including tool results. The canonicalizer now supports `@status`, response headers and the `;req` component parameter (RFC 9421 Section 2.4), so a response can cover `"@method";req`, `"@target-uri";req`, `"@authority";req`, `"content-digest";req` and `"signature";req` of the request it answers; a response presented for a different request, a swapped result body, or a changed status fails verification. `StrictHTTPResponseVerificationOptions` requires `@status`, the response `content-digest` and request binding. `ResponseSigner` is an `http.Handler` middleware that buffers, digests and signs every response of a server. Previously responses could not be signed at all.
...
- `http.HTTPServer.SetMaxBodyBytes`, `websocket.WSServer.SetMaxMessageBytes`, `SetRequireOrigin`; `rfc9421.MaxBodyBytes`; `hpke.ErrInitRejected`, `hpke.ServerOpts.Logger`.
- `jcs.Marshal`, `jcs.Canonicalize`; `did.ParseA2AAgentCardWithProof`; `keys.LowS`, `keys.EncodeRawECDSASignature`.
- `rfc9421.HTTPVerificationOptions.ExpectedDID`, `ExpectedKeyID`, `ExpectedAuthorities`; `rfc9421.KeyIDDID`.
- `rfc9421.HTTPVerifier.SignResponse`, `VerifyResponse`, `Canonicalizer.BuildResponseSignatureBase`, `BodyIntegrityValidator.ValidateResponseContentDigest`, `ResponseSigner`, `StrictHTTPResponseVerificationOptions`, `HTTPVerificationOptions.RequireRequestBinding`; `IsComponentCovered` now compares component parameters (`"@method";req` differs from `"@method"`).
